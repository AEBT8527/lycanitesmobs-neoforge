...
public class PacketManager {
    ...
    public static void onRegisterPayloads(RegisterPayloadHandlersEvent event) {
        ...
        // 26.x: the 3-arg overload only registers the server-side handler; the client side
        // must be supplied too or client mod-load fails validation. Same handler works for both.
        registrar.playBidirectional(LycanitesPayload.TYPE, LycanitesPayload.CODEC, PacketManager::handlePayload, PacketManager::handlePayload);
    }
    ...
    public <MSG> void sendToServer(MSG message) {
        net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(wrap(message));
    }
}
