...
public class PetManager {
    ...
    private void applyPendingEntryNBT() {
        ...
        for (CompoundTag nbtEntry : this.getPendingEntryNBTs()) {
            if (this.host instanceof Player && nbtEntry.getStringOr("Type", "").equalsIgnoreCase("familiar")) {
                if (!PlayerFamiliars.INSTANCE.hasLoadedFamiliar(this.host.getUUID(), nbtEntry.read("UUID", net.minecraft.core.UUIDUtil.CODEC).orElse(null))) {
                    continue;
                }
            }
            PetEntry petEntry = new PetEntry(nbtEntry.read("UUID", net.minecraft.core.UUIDUtil.CODEC).orElse(null), nbtEntry.getStringOr("Type", ""), this.host, nbtEntry.getStringOr("SummonType", ""));
    ...
    public void readFromNBT(CompoundTag nbtTagCompound) {
        ...
        ListTag entryList = nbtTagCompound.getListOrEmpty("PetManager");
        for (int i = 0; i < entryList.size(); ++i) {
            CompoundTag nbtEntry = (CompoundTag) entryList.get(i);
            if (!nbtEntry.read("UUID", net.minecraft.core.UUIDUtil.CODEC).isPresent() && nbtEntry.contains("EntryName")) {
                LMHelperClass.logInfoMessage("[Pets] Converting Pet Entry from older mod version: " + nbtEntry.getString("EntryName") + "...");
                nbtEntry.store("UUID", net.minecraft.core.UUIDUtil.CODEC, UUID.randomUUID());
            }
            if (nbtEntry.read("UUID", net.minecraft.core.UUIDUtil.CODEC).isPresent()) {
                this.addPendingEntryNBT(nbtEntry.read("UUID", net.minecraft.core.UUIDUtil.CODEC).orElse(null), nbtEntry);
            }
            else {
