...
public class FluidManager {
    ...
    public void defineFluids() {
        ...
        Block.Properties waterBlockProperties = Block.Properties.of().mapColor(MapColor.WATER).noCollision().randomTicks().strength(100).noLootTable().replaceable();
        Block.Properties waterBrightBlockProperties = Block.Properties.of().mapColor(MapColor.WATER).noCollision().randomTicks().strength(100).noLootTable().lightLevel(s -> 10).replaceable();
        Block.Properties lavaBlockProperties = Block.Properties.of().mapColor(MapColor.FIRE).noCollision().randomTicks().strength(100).noLootTable().lightLevel(s -> 15).replaceable();
    ...
    public void addFluid(String fluidName, int fluidColor, int density, int viscosity, int temperature, int luminosity, boolean multiply, LiquidBlockFactory blockFactory, BlockBehaviour.Properties blockProperties, String elementName, boolean destroyItems, boolean worldgen) {
        ...
        ObjectManager.addItem(fluidName + "_bucket", () -> new BucketItem(builder.getStillFluid(), com.lycanitesmobs.core.util.helpers.LMHelperClass.itemId(bucketProps, fluidName + "_bucket")));
    ...
    public void forEachFluidBuilder(BiConsumer<String, FluidBuilder> action) {
        this.fluidBuilders.forEach(action);
    }

    public void forEachFluidType(BiConsumer<Identifier, BaseFluidType> action) {
    ...
    public void registerFluidObjects(BiConsumer<Identifier, BaseFlowingFluid> action) {
    ...
    public static class FluidBuilder {
        ...
        public FluidBuilder(String fluidName, Identifier registryLocation) {
            this.baseName = fluidName;
            this.registryName = registryLocation;
        }

        public FluidType createFluidType(Identifier stillTexture, Identifier flowingTexture, Identifier overlayTexture,
