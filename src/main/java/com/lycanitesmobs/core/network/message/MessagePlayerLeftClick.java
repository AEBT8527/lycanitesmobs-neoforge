...
public class MessagePlayerLeftClick {
     ...
     */
    public static void handle(MessagePlayerLeftClick message, Supplier<PacketContext> ctx) {
        ...
        ctx.get().enqueueWork(() -> {
            ...
            if (activeStack.getItem() instanceof ItemEquipment) {
                ((ItemEquipment) activeStack.getItem()).onItemLeftClick(player.level(), player, activeHand);
            }
        });
    }
