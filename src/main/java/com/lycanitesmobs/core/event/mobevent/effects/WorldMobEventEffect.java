	...
	public void onUpdate(Level world, Player player, BlockPos pos, int level, int ticks, int variant) {
		if(ticks == 0) {
			...
			if ("start".equalsIgnoreCase(this.rain)) {
				if (world.getServer() != null) world.getServer().setWeatherParameters(0, 6000, true, false);
			} else if ("stop".equalsIgnoreCase(this.rain)) {
				if (world.getServer() != null) world.getServer().setWeatherParameters(6000, 0, false, false);
			}

			// Lightning:
			if (serverWorldInfo != null) {
				if ("start".equalsIgnoreCase(this.thunder)) {
					if (world.getServer() != null) world.getServer().setWeatherParameters(0, 6000, true, true);
			...
			if (this.dayTime >= 0) {
				int dayTime = 23999;
				long currentTime = LMHelperClass.getDayTime(world);
				...
				for (ServerLevel serverWorld : world.getServer().getAllLevels()) {
					if (serverWorld.getLevelData() instanceof net.minecraft.world.level.storage.ServerLevelData lycSld) lycSld.setDayTimeFraction((float) (((currentTime - excessTime + targetTime) % 24000L) / 24000.0D));
				}
			}
		}
	}
