...
public class MobEventSchedule {
	...
	public boolean canStart(Level world) {
		...
		if(!world.dimension().identifier().toString().equals(this.dimensionId)) {
			return false;
		}

		int time = (int)Math.floor(LMHelperClass.getDayTime(world) % 24000D);
	...
	public void start(ExtendedWorld worldExt) {
		if(this.mobEvent == null) {
			MobEventListener.getInstance().triggerRandomMobEvent(worldExt.getWorld(), worldExt, 1);
			return;
		}
