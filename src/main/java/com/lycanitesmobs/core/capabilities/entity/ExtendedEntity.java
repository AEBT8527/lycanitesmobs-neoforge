...
public class ExtendedEntity {
    ...
    public static final IAttachmentSerializer<ExtendedEntity> SERIALIZER = new IAttachmentSerializer<>() {
        @Override
        public boolean write(ExtendedEntity attachment, net.minecraft.world.level.storage.ValueOutput output) {
            CompoundTag tag = new CompoundTag();
            attachment.writeNBT(tag);
            output.store("LycanitesMobs", CompoundTag.CODEC, tag);
            return true;
        }

        @Override
        public ExtendedEntity read(IAttachmentHolder holder, net.minecraft.world.level.storage.ValueInput input) {
            ExtendedEntity attachment = new ExtendedEntity();
            attachment.readNBT(input.read("LycanitesMobs", CompoundTag.CODEC).orElseGet(CompoundTag::new));
            return attachment;
        }
    };
    ...
    public static ExtendedEntity getForEntity(LivingEntity entity) {
        if (entity == null) {
            // LMHelperClass.logWarningMessage("Tried to access an ExtendedEntity from a null Entity.");
            return null;
        }
        ...
        // Server Side:
        ExtendedEntity iExtendedEntity = entity.getData(com.lycanitesmobs.core.capabilities.LycanitesAttachments.EXTENDED_ENTITY);
        if (iExtendedEntity.getEntity() != entity)
            iExtendedEntity.setEntity(entity);
        return iExtendedEntity;
    }
    ...
    public void onUpdate() {
        ...
        // Force Remove Entity:
        ExtendedEntity.forceRemoveEntityIds = ConfigAdmin.INSTANCE.forceRemoveEntityIds.get();
        if (!this.entity.level().isClientSide() && !ExtendedEntity.getForceRemoveEntityIds().isEmpty() && !this.forceRemoveChecked) {
            LMHelperClass.logDebug("ForceRemoveEntity", "Forced entity removal, checking: " + this.entity.getName());
            for (String forceRemoveID : ExtendedEntity.getForceRemoveEntityIds()) {
                if (forceRemoveID.equalsIgnoreCase(LMHelperClass.convertToResourceLocation(this.entity.getType(), this.entity.level().registryAccess()).toString())) {
        ...
        if (!this.entity.level().getBlockState(this.entity.blockPosition()).isSolid()) {
    ...
    public void updatePickedUpByEntity() {
        ...
        if (!this.entity.level().isClientSide()) {
            ...
            if (weight != null) {
                if (this.entity.hasEffect(ObjectManager.holder(weight))) {
                    this.setPickedUpByEntity(null);
                    return;
                }
            }
        ...
        if (this.pickedUpByEntity != null) {
            ...
            if (!this.entity.level().isClientSide() && this.entity instanceof Player) {
    ...
    public void setPickedUpByEntity(Entity pickedUpByEntity) {
        ...
        if (!this.entity.level().isClientSide()) {
            ...
            LycanitesMobs.PACKET_MANAGER.sendToWorld(message, this.entity.level());
        }
    ...
    public boolean isFeared() {
        ...
        return fear != null && this.entity.hasEffect(ObjectManager.holder(fear));
    }
    ...
    public void readNBT(CompoundTag nbtTagCompound) {
        nbtTagCompound.get("extendedEntity");
    }
