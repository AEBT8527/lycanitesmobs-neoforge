    ...
    public boolean onHitEntity(ItemStack itemStack, LivingEntity target, LivingEntity attacker) {
        ...
        for (int i = 0; i < summonCount; i++) {
            if (attacker.getRandom().nextDouble() <= this.summonChance) {
                try {
                    Entity entity = entityType.create(attacker.level(), net.minecraft.world.entity.EntitySpawnReason.MOB_SUMMONED);
                    entity.snapTo(attacker.blockPosition().getX(), attacker.blockPosition().getY(), attacker.blockPosition().getZ(), attacker.yRotO, 0.0F);
                    if (entity instanceof BaseCreatureEntity) {
                        ...
                        entity.snapTo(spawnPos.getX(), spawnPos.getY(), spawnPos.getZ(), attacker.yRotO, 0.0F);
