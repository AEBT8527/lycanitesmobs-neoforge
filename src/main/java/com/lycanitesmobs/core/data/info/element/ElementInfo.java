...
public class ElementInfo {
    ...
    public void buffEntity(LivingEntity targetEntity, int duration, int amplifier) {
        ...
        for (String buff : this.buffs) {
            MobEffect effect = BuiltInRegistries.MOB_EFFECT.getValue(Identifier.parse(buff));
            if (effect != null) {
                targetEntity.addEffect(new MobEffectInstance(ObjectManager.holder(effect), duration, amplifier));
            }
        }
    }
    ...
    public void debuffEntity(LivingEntity targetEntity, int duration, int amplifier) {
        ...
        for (String debuff : this.debuffs) {
            if ("burning".equalsIgnoreCase(debuff)) {
                targetEntity.igniteForSeconds(duration / 20);
                continue;
            }
    ...
    public boolean isEffectApplicable(MobEffectInstance effect) {
        ...
        Identifier effectKey = BuiltInRegistries.MOB_EFFECT.getKey(effect.getEffect().value());
