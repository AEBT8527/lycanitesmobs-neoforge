...
public class ObjectLists {
    ...
    //                        Add
    // ==================================================
	/** 26.x: ItemStack creation needs bound data components (post registry freeze), so adds
	 * made during mod construction are queued and flushed from common setup. */
	private static final java.util.List<Object[]> lycPendingItems = new ArrayList<>();
	private static boolean lycComponentsReady = false;

	public static void lycFlushPendingItems() {
		if (lycComponentsReady) {
			return;
		}
		try {
			// Components are data-driven in 26.x and bind during server resource load.
			net.minecraft.core.registries.BuiltInRegistries.ITEM.wrapAsHolder(net.minecraft.world.item.Items.STICK).components();
		} catch (Exception e) {
			return; // Not bound yet; keep the queue and retry on next flush call.
		}
		lycComponentsReady = true;
		java.util.List<Object[]> pending = new ArrayList<>(lycPendingItems);
		lycPendingItems.clear();
		for (Object[] entry : pending) {
			addItem((String) entry[0], entry[1]);
		}
	}
	...
	public static void addItem(String list, Object object) {
		if(!(object instanceof Item || object instanceof Block || object instanceof ItemStack || object instanceof String))
			return;
		if (!lycComponentsReady) {
			lycPendingItems.add(new Object[]{list, object});
			return;
		}
	...
	public static void createVanillaLists() {
		...
		// ========== Effects ==========
		// Buffs:
		ObjectLists.addEffect("buffs", MobEffects.STRENGTH.value(), "strength");
		ObjectLists.addEffect("buffs", MobEffects.HASTE.value(), "haste");
		ObjectLists.addEffect("buffs", MobEffects.FIRE_RESISTANCE.value(), "fire_resistance");
		ObjectLists.addEffect("buffs", MobEffects.INSTANT_HEALTH.value(), "instant_health");
		ObjectLists.addEffect("buffs", MobEffects.INVISIBILITY.value(), "invisibility");
		ObjectLists.addEffect("buffs", MobEffects.JUMP_BOOST.value(), "jump_boost");
		ObjectLists.addEffect("buffs", MobEffects.SPEED.value(), "speed");
		...
		ObjectLists.addEffect("buffs", MobEffects.RESISTANCE.value(), "resistance");
        ...
        ObjectLists.addEffect("debuffs", MobEffects.NAUSEA.value(), "nausea");
        ObjectLists.addEffect("debuffs", MobEffects.MINING_FATIGUE.value(), "mining_fatigue");
        ObjectLists.addEffect("debuffs", MobEffects.INSTANT_DAMAGE.value(), "instant_damage");
        ObjectLists.addEffect("debuffs", MobEffects.HUNGER.value(), "hunger");
        ObjectLists.addEffect("debuffs", MobEffects.SLOWNESS.value(), "slowness");
	...
	public static boolean isPickaxe(ItemStack itemStack) {
		...
		if(lycToolPath(itemStack).contains("pickaxe")) {
			return true;
		}
		if(lycToolPath(itemStack).contains("pickaxe")) {
			return true;
	...
	public static boolean isAxe(ItemStack itemStack) {
		...
		if(lycToolPath(itemStack).contains("axe")) {
			return true;
		}
		if(lycToolPath(itemStack).contains("axe")) {
			return true;
	...
	public static boolean isShovel(ItemStack itemStack) {
		...
		if(lycToolPath(itemStack).contains("shovel")) {
			return true;
		}
		if(lycToolPath(itemStack).contains("shovel")) {
			return true;
    ...
    /** 26.x: tool item classes and *_DIG abilities were removed; classify by registry path. */
    private static String lycToolPath(net.minecraft.world.item.ItemStack itemStack) {
        return net.minecraft.core.registries.BuiltInRegistries.ITEM.getKey(itemStack.getItem()).getPath();
    }
