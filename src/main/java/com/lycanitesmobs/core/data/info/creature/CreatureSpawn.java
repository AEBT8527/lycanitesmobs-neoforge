...
public class CreatureSpawn {
    ...
    public void registerVanillaSpawns(CreatureInfo creatureInfo, CreatureSpawnConfig spawnConfig) {
        // Dungeon Spawn:
        if (!spawnConfig.disableDungeonSpawners()) {
            if (this.dungeonWeight > 0) {
                /* NeoForge removed DungeonHooks; vanilla dungeon-spawner injection no longer available. */
    ...
    public boolean isAllowedDimension(Level world) {
        ...
        // Check IDs:
        String targetDimensionId = world.dimension().identifier().toString();
