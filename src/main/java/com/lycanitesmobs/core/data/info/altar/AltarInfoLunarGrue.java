...
public class AltarInfoLunarGrue extends AltarInfo {
    ...
    private boolean checkRotationX(Block bodyBlock, Entity entity, Level world, int x, int y, int z) {
        // Left Arm:
    ...
    private boolean checkRotationZ(Block bodyBlock, Entity entity, Level world, int x, int y, int z) {
        // Left Arm:
    ...
    public boolean activate(Entity entity, Level world, BlockPos pos, int variant) {
        ...
        // Create Mini Boss:
        BaseCreatureEntity entityGrue = (BaseCreatureEntity) CreatureManager.getInstance().getCreature("grue").createEntity(world);
        ...
        // Offset:
        if(entity != null)
            pos = this.getFacingPosition(pos, 10, entity.yRotO);
        ...
        // Spawn Mini Boss:
        entityGrue.markAltarSummoned();
        entityGrue.forceBossHealthBar();
        entityGrue.applyVariant(3);
        entityGrue.snapTo(x, y - 2, z, 0, 0);
        this.spawnDeferred(world, pos, "altar_spawn:" + this.name + ":" + pos.asLong(), entityGrue,
