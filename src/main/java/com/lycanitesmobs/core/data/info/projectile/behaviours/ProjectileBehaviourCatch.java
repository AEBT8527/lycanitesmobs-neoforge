...
public class ProjectileBehaviourCatch extends ProjectileBehaviour {
    ...
    public boolean canDamage(BaseProjectileEntity projectile, Level world, LivingEntity target, boolean canDamage) {
        if (projectile.level().isClientSide() || target == projectile.getOwner()) {
            return canDamage;
        }
        Identifier entityResourceLocation = LMHelperClass.convertToResourceLocation(target.getType(), world.registryAccess());
