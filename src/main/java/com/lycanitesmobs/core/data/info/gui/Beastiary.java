...
public class Beastiary {
    ...
    public boolean addCreatureKnowledge(CreatureKnowledge newKnowledge, boolean sendToClient) {
        ...
        if (sendToClient) {
            ...
            if (!this.extendedPlayer.getPlayer().level().isClientSide()) {
    ...
    public void sendAddedMessage(CreatureKnowledge creatureKnowledge) {
        if (this.extendedPlayer.getPlayer().level().isClientSide() || !CreatureManager.getInstance().getConfig().beastiaryKnowledgeMessages()) {
            return;
        }
        ...
        this.extendedPlayer.getPlayer().sendSystemMessage(message);
        ...
        if (creatureInfo.isSummonable() && creatureKnowledge.getRank() == 2) {
            ...
            this.extendedPlayer.getPlayer().sendSystemMessage(summonMessage);
        }
        ...
        if (creatureInfo.isTameable() && creatureKnowledge.getRank() == 2) {
            ...
            this.extendedPlayer.getPlayer().sendSystemMessage(tameMessage);
        }
    }
    ...
    public void readFromNBT(CompoundTag nbtTagCompound) {
        ...
        ListTag knowledgeList = nbtTagCompound.getListOrEmpty("CreatureKnowledge");
        for (int i = 0; i < knowledgeList.size(); ++i) {
            CompoundTag nbtKnowledge = knowledgeList.getCompoundOrEmpty(i);
            if (nbtKnowledge.contains("CreatureName")) {
                String creatureName = nbtKnowledge.getStringOr("CreatureName", "");
                int rank = 0;
                if (nbtKnowledge.contains("Rank")) {
                    rank = nbtKnowledge.getIntOr("Rank", 0);
                }
                int experience = 0;
                if (nbtKnowledge.contains("Experience")) {
                    experience = nbtKnowledge.getIntOr("Experience", 0);
                }
