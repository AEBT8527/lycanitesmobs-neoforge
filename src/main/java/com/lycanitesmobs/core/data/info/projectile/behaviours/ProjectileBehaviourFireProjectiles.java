...
public class ProjectileBehaviourFireProjectiles extends ProjectileBehaviour {
    ...
    public void onProjectileUpdate(BaseProjectileEntity projectile) {
        if (this.tickRate < 0 || projectile.getUpdateTick() % this.tickRate != 0 || projectile.level().isClientSide()) {
            return;
        }
    ...
    public BaseProjectileEntity createProjectile(BaseProjectileEntity projectile) {
        ...
        if (projectile.getOwner() != null) {
            ...
            childProjectile = projectileInfo.createProjectile(projectile.level(), shooter);
        ...
        childProjectile.shoot(
                motionX + (projectile.level().getRandom().nextGaussian() - 0.5D),
                motionY + (projectile.level().getRandom().nextGaussian() - 0.5D),
                motionZ + (projectile.level().getRandom().nextGaussian() - 0.5D),
                this.velocity,
                0
        );

        projectile.playSound(childProjectile.getLaunchSound(), 1.0F, 1.0F / (projectile.level().getRandom().nextFloat() * 0.4F + 0.8F));
        DeferredLevelActionManager.spawnEntity(projectile.level(), projectile.blockPosition(), null, childProjectile);
