...
public class JSONHelper {
    ...
    public static List<Block> getJsonBlocks(JsonArray jsonArray) {
        ...
        while (jsonIterator.hasNext()) {
            Block block = BuiltInRegistries.BLOCK.getValue(Identifier.parse(jsonIterator.next().getAsString()));
            if (block != null) {
                blocks.add(block);
            }
        }
    ...
    public static List<Item> getJsonItems(JsonArray jsonArray) {
        ...
        Iterator<JsonElement> jsonIterator = jsonArray.iterator();
        while (jsonIterator.hasNext()) {
            Item item = BuiltInRegistries.ITEM.getValue(Identifier.parse(jsonIterator.next().getAsString()));
            if (item != null) {
                items.add(item);
            }
        }
    ...
    public static List<String> getBiomesFromTags(Level world, List<String> biomeTags) {
        ...
        for (String rawEntry : biomeTags) {
            ...
            Identifier tagLoc = Identifier.parse(entry);
            ...
            // NeoForge unifies Forge and vanilla tags, so the vanilla registry tag lookup covers
            // everything the old Forge ITagManager fallback used to. No separate Forge path needed.
            var vanillaSetOpt = biomeRegistry.get(vanillaKey);
    ...
    public static List<Biome> getBiomes(Level world, List<String> biomeIds) {
        List<Biome> biomes = new ArrayList<>();
        var biomeRegistry = world.registryAccess().lookupOrThrow(Registries.BIOME);
        for (String biomeId : biomeIds) {
            Biome biome = biomeRegistry.get(net.minecraft.resources.ResourceKey.create(Registries.BIOME, Identifier.parse(biomeId))).map(net.minecraft.core.Holder::value).orElse(null);
            if (biome != null) {
                biomes.add(biome);
            }
        }
