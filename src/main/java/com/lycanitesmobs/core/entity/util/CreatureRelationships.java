...
public class CreatureRelationships {
	...
	public List<Player> getPlayers() {
		...
		if (this.getCreatureEntity().level().isClientSide()) {
			return players;
		}

		for (CreatureRelationshipEntry relationshipEntry : this.relationships.values()) {
			Player player = this.getCreatureEntity().level().getPlayerByUUID(relationshipEntry.getTargetEntityUUID());
			if (player != null) {
				players.add(player);
			}
		}
	...
	public void load(CompoundTag nbt) {
		if(nbt.contains("CreatureRelationships")) {
			...
			ListTag relationshipNbtList = nbt.getListOrEmpty("CreatureRelationships");
			for(int i = 0; i < relationshipNbtList.size(); i++) {
				CompoundTag relationshipNbt = relationshipNbtList.getCompoundOrEmpty(i);
