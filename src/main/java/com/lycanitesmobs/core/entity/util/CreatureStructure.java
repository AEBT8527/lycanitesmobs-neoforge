...
public class CreatureStructure {
			...
			for (int y = -radius; y <= radius; y++) {
				for (int z = -radius; z <= radius; z++) {
					...
					// Inside Air:
						...
						BlockState targetState = this.owner.level().getBlockState(blockPos);
						if (targetState.getBlock() != Blocks.AIR && !(targetState.getBlock() instanceof net.minecraft.world.level.block.LiquidBlock)) {
							blockState = Blocks.AIR.defaultBlockState();
						}
					}
					...
					if (blockState != null && this.owner.level().getBlockState(blockPos).getBlock() != blockState.getBlock()) {
				...
				for (int z = -radius; z <= radius; z++) {
					...
					for (int pitLayer = 1; pitLayer <= (radius / 2); pitLayer++) {
						if (y == radius - pitLayer && Math.abs(x) < (radius - pitLayer) && Math.abs(z) < (radius - pitLayer)) {
							...
							if (this.owner.level().getBlockState(blockPos).getBlock() != blockState.getBlock()) {
	...
	protected boolean shouldBuildAt(BlockPos pos) {
		BlockState targetState = this.owner.level().getBlockState(pos);
		Block targetBlock = targetState.getBlock();
		if (targetBlock == Blocks.AIR) {
			return true;
		}
		if (this.owner.level().getBlockEntity(pos) != null) {
			return false;
		}
