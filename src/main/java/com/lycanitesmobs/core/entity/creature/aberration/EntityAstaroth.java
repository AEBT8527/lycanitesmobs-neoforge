    ...
    public void aiStep() {
        ...
        if (this.updateTick % 20 == 0) {
            if (this.getMasterTarget() != null && this.getMasterTarget() instanceof EntityAsmodeus && ((BaseCreatureEntity) this.getMasterTarget()).getBattlePhase() > 0) {
                EntityHellShield projectile = new EntityHellShield(ProjectileManager.getInstance().getOldProjectileType(EntityHellShield.class), this.level(), this);
    ...
    public void die(DamageSource damageSource) {
        if (!this.level().isClientSide() && CreatureManager.getInstance().getCreature("trite").isEnabled()) {
            ...
            for (int k = 0; k < j; ++k) {
                ...
                this.summonMinion(trite, this.getRandom().nextFloat() * 360.0F, 0.5F);
                if (this.isTamed()) {
