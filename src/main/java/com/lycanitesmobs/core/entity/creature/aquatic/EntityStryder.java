...
public class EntityStryder extends RideableCreatureEntity implements IGroupHeavy {
    ...
    public EntityStryder(EntityType<? extends EntityStryder> entityType, Level world) {
        ...
        this.getAttribute(net.minecraft.world.entity.ai.attributes.Attributes.STEP_HEIGHT).setBaseValue(4.0F);
    }
                ...
                if (this.isTamed() && !this.canAttack(this.getPickupEntity())) {
                    ...
                    if (this.getPickupEntity() instanceof LivingEntity) {
                        if (ObjectManager.getEffect("penetration") != null)
                            this.getPickupEntity().addEffect(new MobEffectInstance(ObjectManager.holder(ObjectManager.getEffect("penetration")), this.getEffectDuration(5), 1));
                    }
                }
            } else {
    ...
    public BlockPos getWanderPosition(BlockPos wanderPosition) {
        BlockPos groundPos;
        for (groundPos = wanderPosition.below(); groundPos.getY() > 0 && !this.level().getBlockState(groundPos).isSolid(); groundPos = groundPos.below()) {
        }
