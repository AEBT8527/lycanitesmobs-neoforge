...
public class EntityRoc extends RideableCreatureEntity implements Enemy {
    private AttackMeleeGoal attackAI;
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(this.claimCombatGoalIndex(), this.attackAI);
    }
    ...
    public void aiStep() {
        ...
        if(!this.level().isClientSide() && this.getControllingPassenger() == null) {
	    	...
	    	if(this.hasPickupEntity()) {
                ...
                if(this.getPickupEntity() instanceof Creeper && this.hasAttackTarget() && !(this.getTarget() instanceof Creeper)) {
                    double distance = new Vector3d(this.position().x(), 0, this.position().z()).distance(new Vector3d(this.getTarget().position().x(), 0, this.getTarget().position().z()));
                    ...
                    if (this.tickCount % 100 == 0 && this.getRandom().nextBoolean()) {
                        if (this.getPickupEntity() instanceof Player) {
                            for (int distToGround = 0; distToGround < 8; distToGround++) {
                                Block searchBlock = this.level().getBlockState(new BlockPos((int) this.position().x(), (int) this.position().y() - distToGround, (int) this.position().z())).getBlock();
        ...
        if(!this.level().isClientSide() && this.getControllingPassenger() == null && this.getPickupEntity() instanceof Creeper) {
    ...
    public boolean isAggressive() {
        ...
        if (!this.hasSpawnEvent() && this.level().isBrightOutside() && this.testLightLevel() >= 2) {
            return false;
        }
        return super.isAggressive();
    }
    ...
    public void pickupEntity(LivingEntity entity) {
        ...
        if(this.level().getBlockState(this.blockPosition()) != null && this.level().canSeeSkyFromBelowWater(this.blockPosition()))
            this.leap(0.5F, 4.0D);
    }
