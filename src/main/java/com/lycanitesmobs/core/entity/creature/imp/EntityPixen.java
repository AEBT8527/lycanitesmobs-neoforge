...
public class EntityPixen extends TameableCreatureEntity implements Enemy {
    ...
    public void aiStep() {
        ...
        // Mischief Aura:
        if(!this.level().isClientSide() && this.auraRate > 0 && !this.isPetType("familiar")) {
            if (this.updateTick % this.auraRate == 0) {
                ...
                for (Object entityObj : aoeTargets) {
                    ...
                    if (target != this && !(target instanceof EntityPixen) && target != this.getTarget() && target != this.getAvoidTarget()) {
                        ...
                        MobEffect effect = BuiltInRegistries.MOB_EFFECT.getValue(Identifier.parse(this.auraEffects.get(randomIndex)));
                        if(effect != null) {
                            target.addEffect(new MobEffectInstance(ObjectManager.holder(effect), this.auraDuration, this.auraAmplifier));
                        }
                    }
                }
            }
