...
public class EntityConba extends TameableCreatureEntity implements Enemy {
    ...
    public void aiStep() {
        ...
        if (!this.level().isClientSide()) {
            ...
            if (this.vespidInfection && !this.level().isClientSide()) {
    ...
    public void die(DamageSource damageSource) {
        ...
        if (!this.level().isClientSide() && this.vespidInfection)
            this.spawnVespidSwarm();
    }

    public void spawnVespidSwarm() {
        int j = 2 + this.getRandom().nextInt(5) + level().getDifficulty().getId() - 1;
        for (int k = 0; k < j; ++k) {
            ...
            EntityVespid vespid = (EntityVespid) CreatureManager.getInstance().getCreature("vespid").createEntity(this.level());
            vespid.snapTo(this.position().x() + (double) f, this.position().y() + 0.5D, this.position().z() + (double) f1, this.getRandom().nextFloat() * 360.0F, 0.0F);
            ...
            DeferredLevelActionManager.spawnEntity(this.level(), this.blockPosition(), null, vespid);
    ...
    public void readAdditionalSaveData(CompoundTag nbt) {
        ...
        if (nbt.contains("VespidInfection")) {
            this.vespidInfection = nbt.getBooleanOr("VespidInfection", false);
        }
        if (nbt.contains("VespidInfectionTime")) {
            this.vespidInfectionTime = nbt.getIntOr("VespidInfectionTime", 0);
        }
    }
