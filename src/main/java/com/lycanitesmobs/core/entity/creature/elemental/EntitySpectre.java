...
public class EntitySpectre extends TameableCreatureEntity implements Enemy, IGroupHeavy {
    ...
    public void aiStep() {
        ...
        if (!this.level().isClientSide()) {
            ...
            if (this.canPull()) {
                for (LivingEntity entity : this.getNearbyEntities(LivingEntity.class, null, this.pullRange)) {
                    if (entity == this || entity == this.getControllingPassenger() || entity instanceof IGroupBoss || entity instanceof IGroupHeavy || entity.hasEffect(ObjectManager.holder(ObjectManager.getEffect("weight"))) || !this.canAttack(entity))
                        continue;
                    ...
                    double xzDist = Mth.sqrt(LMHelperClass.convertToFloat(xDist * xDist + zDist * zDist));
                    double factor = 0.1D;
                    double motionCap = 10;
        ...
        if (this.level().isClientSide())
            for (int i = 0; i < 2; ++i) {
                this.level().addParticle(ParticleTypes.PORTAL, this.position().x() + (this.getRandom().nextDouble() - 0.5D) * (double) this.getDimensions(Pose.STANDING).width(), this.position().y() + this.getRandom().nextDouble() * (double) this.getDimensions(Pose.STANDING).height(), this.position().z() + (this.getRandom().nextDouble() - 0.5D) * (double) this.getDimensions(Pose.STANDING).width(), 0.0D, 0.0D, 0.0D);
            }
    }
