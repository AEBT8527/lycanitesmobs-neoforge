...
public class EntityBobeko extends AgeableCreatureEntity {
    ...
    public void aiStep() {
        ...
        // Trail:
        if (!this.level().isClientSide() && (this.tickCount % 10 == 0 || this.isMoving() && this.tickCount % 5 == 0)) {
            int trailHeight = 2;
            if (this.isBaby())
                trailHeight = 1;
            for (int y = 0; y < trailHeight; y++) {
                ...
                if (block == Blocks.AIR || block == Blocks.SNOW || block == ObjectManager.getBlock("frostcloud"))
                    this.level().setBlockAndUpdate(this.blockPosition().offset(0, y, 0), ObjectManager.getBlock("frostcloud").defaultBlockState());
            }
        }
    ...
    public boolean canBeAffected(MobEffectInstance potionEffect) {
        if (potionEffect.getEffect() == MobEffects.SLOWNESS) return false;
