...
public class EntityConcapedeHead extends AgeableCreatureEntity {
	...
	public void onFirstSpawn() {
        ...
        if(!this.level().isClientSide() && this.backSegment == null) {
        	...
        	for(int segment = 0; segment < segmentCount; segment++) {
        		EntityConcapedeSegment segmentEntity = (EntityConcapedeSegment)CreatureManager.getInstance().getCreature("concapedesegment").createEntity(parentSegment.level());
        		segmentEntity.snapTo(parentSegment.position().x(), parentSegment.position().y(), parentSegment.position().z(), 0.0F, 0.0F);
                ...
                DeferredLevelActionManager.spawnEntity(parentSegment.level(), segmentEntity.blockPosition(), null, segmentEntity);
	...
	public void setGrowingAge(int age) {
		...
		if(!this.level().isClientSide() && !this.isFirstSpawn() && age == 0 && !this.isHungry && CreatureManager.getInstance().getCreature("concapedesegment") != null) {
			...
			if(size < CONCAPEDE_SIZE_MAX) {
				EntityConcapedeSegment segmentEntity = (EntityConcapedeSegment)CreatureManager.getInstance().getCreature("concapedesegment").createEntity(lastSegment.level());
	    		segmentEntity.snapTo(lastSegment.position().x(), lastSegment.position().y(), lastSegment.position().z(), 0.0F, 0.0F);
				segmentEntity.setParentTarget(lastSegment);
				segmentEntity.applyVariant(this.getVariantIndex());
				segmentEntity.setSizeScale(this.sizeScale);
				segmentEntity.inheritSpawnEventFrom(this);
				segmentEntity.markNotFirstSpawn();
                DeferredLevelActionManager.spawnEntity(lastSegment.level(), segmentEntity.blockPosition(), null, segmentEntity);
			}
		}
	...
	public void readAdditionalSaveData(CompoundTag nbt) {
		if(nbt.contains("IsHungry")) {
			this.isHungry = nbt.getBooleanOr("IsHungry", false);
		}
