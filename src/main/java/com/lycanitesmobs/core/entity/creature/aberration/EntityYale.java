...
public class EntityYale extends AgeableCreatureEntity implements IShearable {
	...
	@Override
	public ArrayList<ItemStack> onSheared(@Nullable Player player, @Nonnull ItemStack item, Level world, BlockPos pos) {
		...
		int quantity = this.woolDrop.getQuantity(this.getRandom(), 0, 1);
	...
	private DyeColor getMixedFurColor(BaseCreatureEntity father, BaseCreatureEntity mother) {
		...
		TransientCraftingContainer craftinginventory = mixColors(dyeA, dyeB);
		var craftInput = craftinginventory.asCraftInput();
		return this.level().getRecipeManager().getRecipeFor(RecipeType.CRAFTING, craftInput, this.level()).map((recipeHolder) ->
				recipeHolder.value().assemble(craftInput, level().registryAccess())).map(ItemStack::getItem).filter(DyeItem.class::isInstance).map(DyeItem.class::cast).map(DyeItem::getDyeColor).orElseGet(() ->
				this.level().random.nextBoolean() ? dyeA : dyeB);
	}

	private static TransientCraftingContainer mixColors(DyeColor dyeA, DyeColor dyeB) {
	...
	private static float[] createSheepColor(DyeColor p_192020_0_) {
		if (p_192020_0_ == DyeColor.WHITE) {
			...
			int dyeRgb = p_192020_0_.getTextureDiffuseColor();
			return new float[]{
					net.minecraft.util.FastColor.ARGB32.red(dyeRgb) / 255.0F * 0.75F,
					net.minecraft.util.FastColor.ARGB32.green(dyeRgb) / 255.0F * 0.75F,
					net.minecraft.util.FastColor.ARGB32.blue(dyeRgb) / 255.0F * 0.75F};
		}
	}
