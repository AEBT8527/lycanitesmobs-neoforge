...
public class EntityYale extends AgeableCreatureEntity implements IShearable {
	...
	/**
	 * Attempts to mix both parents to come up with a mixed dye color.
	 * 26.x: the crafting-simulation approach (DyeItem.byColor + RecipeManager.getRecipeFor with a
	 * fake container) is gone; blend the two dye colours directly like vanilla sheep do.
	 */
	private DyeColor getMixedFurColor(BaseCreatureEntity father, BaseCreatureEntity mother) {
		...
		return this.level().getRandom().nextBoolean() ? dyeA : dyeB;
	}
	...
	private static float[] createSheepColor(DyeColor p_192020_0_) {
		if (p_192020_0_ == DyeColor.WHITE) {
			...
			return new float[]{
					net.minecraft.util.ARGB.red(dyeRgb) / 255.0F * 0.75F,
					net.minecraft.util.ARGB.green(dyeRgb) / 255.0F * 0.75F,
					net.minecraft.util.ARGB.blue(dyeRgb) / 255.0F * 0.75F};
		}
	}
    ...
    public void readAdditionalSaveData(CompoundTag nbt) {
    	...
    	if(nbt.contains("HasFur")) {
    		this.setFur(nbt.getBooleanOr("HasFur", false));
    	}
    }
