...
public class EntityFear extends BaseCreatureEntity {
    ...
    public static void spawnForPlayer(Player player, @Nullable LivingEntity source) {
        ...
        if (fear == null || !player.hasEffect(ObjectManager.holder(fear))) return;

        // Expected count = amplitude + 1 (fear 0 → 1 entity, fear 1 → 2, etc.)
        int expectedCount = player.getEffect(ObjectManager.holder(fear)).getAmplifier() + 1;
        ...
        EntityFear fearEntity = (EntityFear) fearInfo.getEntityType().create(world, net.minecraft.world.entity.EntitySpawnReason.MOB_SUMMONED);
    ...
    public EntityFear(EntityType<? extends EntityFear> entityType, Level world) {
        ...
        this.hauntTicksRemaining = 300 + this.getRandom().nextInt(301); // 15-30 seconds
    ...
    public void setHauntTarget(@Nullable Player player) {
        ...
        if (player != null) {
            this.snapTo(player.getX(), player.getY() + 1.5, player.getZ(), 0, 0);
        }
    }
    ...
    public void aiStep() {
        ...
        if (this.hauntTarget != null && !this.hauntTarget.level().equals(this.level())) {
        ...
        MobEffect fear = ObjectManager.getEffect("fear");
        if (fear == null || !this.hauntTarget.hasEffect(ObjectManager.holder(fear))) {
            spawnCooldowns.remove(this.hauntTarget.getUUID());
            this.discard();
            return;
        }
        ...
        if (distSq > 30 * 30) {
            this.snapTo(this.hauntTarget.getX(), this.hauntTarget.getY() + 1.5, this.hauntTarget.getZ(), 0, 0);
        }
    }
    ...
    public List<Player> getFearedPlayersNearby(double range) {
        ...
        return this.level().getEntitiesOfClass(Player.class, box, p -> {
            if (!p.isAlive()) return false;
            if (fear == null) return false;
            return p.hasEffect(ObjectManager.holder(fear));
        });
    }
    ...
    /** Cannot follow players through portals. (26.x: hook removed from the extension interface.) */
