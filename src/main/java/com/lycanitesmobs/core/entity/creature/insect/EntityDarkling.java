...
public class EntityDarkling extends TameableCreatureEntity implements Enemy {
    ...
    public void aiStep() {
        ...
        // Leap:
        if (!this.level().isClientSide() && this.hasAttackTarget() && !this.hasLatchTarget() && this.onGround() && !this.level().isClientSide() && this.getRandom().nextInt(10) == 0)
        ...
        if (this.hasLatchTarget()) {
            ...
            // Client:
                ...
                for (int i = 0; i < 2; ++i) {
                    this.level().addParticle(DustParticleOptions.REDSTONE, this.position().x() + (this.getRandom().nextDouble() - 0.5D) * (double) this.getDimensions(Pose.STANDING).width(), this.position().y() + this.getRandom().nextDouble() * (double) this.getDimensions(Pose.STANDING).height(), this.position().z() + (this.getRandom().nextDouble() - 0.5D) * (double) this.getDimensions(Pose.STANDING).width(), 0.0D, 0.0D, 0.0D);
                }
            }
        } else
            this.noPhysics = false;
        ...
        try {
            if (this.level().isClientSide()) {
                ...
                if (latchEntityID != this.latchEntityID) {
                    ...
                    if (latchEntityID != 0) {
                        Entity possilbeLatchEntity = this.level().getEntity(latchEntityID);
    ...
    public void setLatchTarget(LivingEntity entity) {
        ...
        if (ObjectManager.getEffect("repulsion") != null) {
            if ((entity).hasEffect(ObjectManager.holder(ObjectManager.getEffect("repulsion")))) {
                this.getEntityData().set(LATCH_TARGET, 0);
                this.latchEntity = null;
                return;
            }
