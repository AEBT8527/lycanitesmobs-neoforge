...
public class EntityWendigo extends BaseCreatureEntity implements Enemy {
                    ...
                    if (trailWidth == 1)
                        ...
                        for (int x = -(trailWidth / 2); x < (trailWidth / 2) + 1; x++) {
                            for (int z = -(trailWidth / 2); z < (trailWidth / 2) + 1; z++) {
                                this.level().setBlockAndUpdate(this.blockPosition().offset(x, y, z), ObjectManager.getBlock("frostfire").defaultBlockState());
                            }
                        }
                }
            }
    ...
    public float getBlockPathWeight(int x, int y, int z) {
        ...
        if (ObjectManager.getBlock("ooze") != null) {
            if (this.level().getBlockState(pos).getBlock() == ObjectManager.getBlock("ooze"))
    ...
    public boolean isInvulnerableTo(net.minecraft.server.level.ServerLevel lycLevel, DamageSource source) {
        return ObjectManager.getDamageSource(this.level(), "ooze").type().equals(source.type()) || super.isInvulnerableTo(lycLevel, source);
    }
