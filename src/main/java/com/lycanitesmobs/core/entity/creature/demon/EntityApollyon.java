...
//        this.goalSelector.addGoal(this.currentCombatGoalIndex(), new BuildAroundTargetGoal(this).setBlock(ObjectManager.getBlock("doomfire")).setTickRate(40).setRange(3).setEnclose(true).setTargetBit(TARGET_BITS.ATTACK));
        ...
        this.goalSelector.addGoal(this.currentCombatGoalIndex(), new EffectAuraGoal(this).setEffect(MobEffects.DAMAGE_BOOST.value()).setAmplifier(2).setEffectSeconds(2).setRange(32).setCheckSight(false)
                .setTargetTypes(TARGET_TYPES.ALLY.id).setTargetCreatureType("demon"));
        this.goalSelector.addGoal(this.currentCombatGoalIndex(), new EffectAuraGoal(this).setEffect(MobEffects.MOVEMENT_SPEED.value()).setAmplifier(2).setEffectSeconds(2).setRange(32).setCheckSight(false)
                .setTargetTypes(TARGET_TYPES.ALLY.id).setTargetCreatureType("demon"));
        this.goalSelector.addGoal(this.currentCombatGoalIndex(), new EffectAuraGoal(this).setEffect(MobEffects.DAMAGE_RESISTANCE.value()).setAmplifier(2).setEffectSeconds(2).setRange(32).setCheckSight(false)
                .setTargetTypes(TARGET_TYPES.ALLY.id).setTargetCreatureType("demon"));
