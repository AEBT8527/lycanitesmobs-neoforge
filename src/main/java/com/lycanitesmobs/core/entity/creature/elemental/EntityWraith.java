...
public class EntityWraith extends TameableCreatureEntity implements Enemy {
    ...
    public void aiStep() {
        ...
        if(!this.level().isClientSide()) {
            if(this.detonateTimer == 0) {
                this.level().explode(this, this.position().x(), this.position().y(), this.position().z(), 1, Level.ExplosionInteraction.TNT);
        ...
        // Particles:
        if(this.level().isClientSide() && this.detonateTimer <= 5) {
			for (int i = 0; i < 2; ++i) {
    ...
    public void die(DamageSource par1DamageSource) {
		if(!this.level().isClientSide() && LMHelperClass.getGameRuleBool(this.level(), GameRules.MOB_GRIEFING, true)) {
			int explosionRadius = 1;
			if(this.subspecies != null)
				explosionRadius = 3;
			explosionRadius = Math.max(1, Math.round((float)explosionRadius * (float)this.sizeScale));
			this.level().explode(this, this.position().x(), this.position().y(), this.position().z(), explosionRadius, Level.ExplosionInteraction.TNT);
		}
	...
	public void readAdditionalSaveData(CompoundTag nbt) {
		...
		if(nbt.contains("DetonateTimer")) {
			this.detonateTimer = nbt.getIntOr("DetonateTimer", 0);
		}
	}
