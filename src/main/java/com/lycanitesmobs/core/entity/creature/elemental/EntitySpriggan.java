...
public class EntitySpriggan extends TameableCreatureEntity implements Enemy {
    ...
    public void aiStep() {
            ...
            for (int x = (int) this.position().x() - farmingRange; x <= (int) this.position().x() + farmingRange; x++) {
                for (int y = (int) this.position().y() - farmingHeight; y <= (int) this.position().y() + farmingHeight; y++) {
                    for (int z = (int) this.position().z() - farmingRange; z <= (int) this.position().z() + farmingRange; z++) {
                        ...
                        Block farmingBlock = this.level().getBlockState(pos).getBlock();
                        if (farmingBlock instanceof net.minecraft.world.level.block.BushBlock && farmingBlock instanceof BonemealableBlock && farmingBlock != Blocks.TALL_GRASS) {

                            // Boost Crops Every X Seconds:
                            if (!this.level().isClientSide() && this.level() instanceof ServerLevel && this.farmingTick % (currentFarmingRate) == 0) {
                                ...
                                if (growableBlock.isValidBonemealTarget(this.level(), pos, this.level().getBlockState(pos))) {
                                    if (growableBlock.isBonemealSuccess(this.level(), this.getRandom(), pos, this.level().getBlockState(pos))) {
                                        growableBlock.performBonemeal((ServerLevel) this.level(), this.getRandom(), pos, this.level().getBlockState(pos));
                                    }
                                }
                            }

                            // Crop Growth Effect:
                            if (this.level().isClientSide() && this.farmingTick % 40 == 0) {
                                double d0 = this.getRandom().nextGaussian() * 0.02D;
                                double d1 = this.getRandom().nextGaussian() * 0.02D;
                                double d2 = this.getRandom().nextGaussian() * 0.02D;
                                this.level().addParticle(ParticleTypes.HAPPY_VILLAGER, (double) ((float) x + this.getRandom().nextFloat()), (double) y + (double) this.getRandom().nextFloat(), (double) ((float) z + this.getRandom().nextFloat()), d0, d1, d2);
                            }
                        }
                    }
                }
        ...
        if (this.level().isClientSide() && !CreatureManager.getInstance().getConfig().disableBlockParticles())
            for (int i = 0; i < 2; ++i) {
                this.level().addParticle(new BlockParticleOption(ParticleTypes.BLOCK, Blocks.TALL_GRASS.defaultBlockState()),
