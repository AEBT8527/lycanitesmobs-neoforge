...
public class GetBlockGoal extends Goal {
    ...
    private Block targetBlock = Blocks.TORCH;
    private String targetBlockName = "";
    private boolean tamedLooting = true;
    ...
    public boolean canUse() {
    	if(!this.host.canPickupItems() || !LMHelperClass.getGameRuleBool(this.host.level(), GameRules.MOB_GRIEFING, true))
    		return false;
        ...
        for(int x = (int)this.host.position().x() - this.distanceMax; x < (int)this.host.position().x() + this.distanceMax; x++) {
        	for(int y = (int)this.host.position().y() - heightDistance; y < (int)this.host.position().y() + heightDistance; y++) {
        		for(int z = (int)this.host.position().z() - this.distanceMax; z < (int)this.host.position().z() + this.distanceMax; z++) {
        			Block searchBlock = this.host.level().getBlockState(new BlockPos(x, y, z)).getBlock();
    ...
    public void tick() {
        if(this.updateRate-- <= 0) {
        	...
        	if(!this.host.useDirectNavigator())
        		...
        		this.host.setDirectNavigationTarget(this.target, this.speed);
        }
    }
}
