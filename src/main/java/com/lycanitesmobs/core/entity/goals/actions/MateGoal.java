...
public class MateGoal extends Goal {
    ...
    public void tick() {
        ...
        if(!this.host.useDirectNavigator())
        	...
        	this.host.setDirectNavigationTarget(new BlockPos((int)this.partner.position().x(), (int)this.partner.position().y(), (int)this.partner.position().z()), speed);
    ...
    private AgeableCreatureEntity getPartner() {
        float distance = 8.0F;
        List possibleMates = this.host.level().getEntitiesOfClass(this.targetClass, this.host.getBoundingBox().inflate((double)distance, (double)distance, (double)distance));
