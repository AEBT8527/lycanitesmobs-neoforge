...
public class GetItemGoal extends Goal {
    ...
    public boolean canUse() {
        ...
        List<ItemEntity> possibleTargets = this.host.level().getEntitiesOfClass(ItemEntity.class, this.host.getBoundingBox().inflate(this.distanceMax, heightDistance, this.distanceMax), this.targetSelector);
    ...
    public void tick() {
        if(this.updateRate-- <= 0) {
        	...
        	if(!this.host.useDirectNavigator())
        		...
        		this.host.setDirectNavigationTarget(new BlockPos((int)this.target.position().x(), (int)this.target.position().y(), (int)this.target.position().z()), this.speed);
        }
    }
}
