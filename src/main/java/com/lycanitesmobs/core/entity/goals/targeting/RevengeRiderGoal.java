...
public class RevengeRiderGoal extends FindAttackTargetGoal {
    ...
    public void start() {
        ...
        try {
            if (this.callForHelp) {
                ...
                List allies = this.host.level().getEntitiesOfClass(this.host.getClass(), this.host.getBoundingBox().inflate(d0, 4.0D, d0), (Predicate<Entity>) input -> input instanceof LivingEntity);
    ...
    public LivingEntity getRiderRevengeTarget() {
        ...
        if (extendedEntity != null) {
            return extendedEntity.getLastAttackedEntity();
        }
        return null;
    }
    ...
    public int getRiderRevengeTime() {
        ExtendedEntity extendedEntity = ExtendedEntity.getForEntity(this.host.getRider());
        if (extendedEntity != null) {
            return extendedEntity.getLastAttackedTime();
        }
        return 0;
    }
}
