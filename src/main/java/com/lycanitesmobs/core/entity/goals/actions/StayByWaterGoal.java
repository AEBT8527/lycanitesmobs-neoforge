...
public class StayByWaterGoal extends Goal {
    ...
    public boolean canUse() {
        ...
        if (inWater) {
            BlockState blockState = this.host.level().getBlockState(this.host.blockPosition());
            ...
            FluidState fluidState = this.host.level().getFluidState(this.host.blockPosition());
                                    ...
                                    if (!enoughSpace) {
                                        ...
                                        for (int adjX = searchX - neededSpace; adjX <= searchX + neededSpace; ++adjX) {
                                            for (int adjZ = searchZ - neededSpace; adjZ <= searchZ + neededSpace; ++adjZ) {
                                                if (this.host.level().getBlockState(new BlockPos(adjX, searchY, adjZ)).isSolid()) {
                                                    enoughSpace = false;
                                                    break;
                                                }
                                            }
    ...
    public boolean isValidWaterPosition(BlockPos pos) {
        if (!this.host.canBreatheUnderwaterCreature())
            return false;
        if (!this.host.waterDamage()) {
            if (this.host.level().getBlockState(pos).getBlock() == Blocks.WATER)
                return true;
        }
        if (com.lycanitesmobs.core.util.helpers.LMHelperClass.isInvulnerableTo(this.host, this.host.level().damageSources().onFire()) || this.host.isLavaCreature()) {
            if (this.host.level().getBlockState(pos).getBlock() == Blocks.LAVA)
                return true;
        }
        return false;
    }
    ...
    public void tick() {
        if (this.updateRate-- <= 0) {
            ...
            if (!host.useDirectNavigator()) {
                ...
                host.setDirectNavigationTarget(this.waterPos, this.speed);
        }
    }
