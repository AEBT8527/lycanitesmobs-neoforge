...
public class FireProjectilesGoal extends BaseGoal {
    ...
    protected Vector3d offset = new Vector3d(0.0, 0.0, 0.0);
    ...
    public void tick() {
        ...
        this.cooldownTime = Math.round((float) ((1.0D / this.host.getAttribute(BaseCreatureEntity.attrHolder(BaseCreatureEntity.RANGED_SPEED)).getValue()) * this.fireRate));

        // All Players Mode:
        if (this.allPlayers) {
            this.host.forEachPlayerTarget(target -> {
    ...
    public BaseProjectileEntity createProjectile() {
        ...
        if (this.projectileName != null) {
            ...
            projectile = projectileInfo.createProjectile(this.host.level(), this.host);
        } else if (this.projectileClass != null) {
            projectile = ProjectileManager.getInstance().createOldProjectile(this.projectileClass, this.host.level(), this.host);
        }
    ...
    public void fireProjectileOverhead(BaseProjectileEntity projectile) {
        ...
        DeferredLevelActionManager.spawnEntity(this.host.level(), target.blockPosition(), null, projectile);
    }
}
