...
public class EatBlockGoal extends Goal {
    ...
    public boolean canUse() {
         ...
         BlockState blockState = this.host.level().getBlockState(new BlockPos(i, j - 1, k));
    ...
    public void tick() {
         ...
         if(this.isValidBlock(blockState)) {
        	 ...
        	 this.host.level().removeBlock(new BlockPos(i, j - 1, k), true); // Might be something else was x, y, z, false
         }

         this.host.level().levelEvent(2001, new BlockPos(i, j - 1, k), Block.getId(blockState));
         this.host.level().setBlock(new BlockPos(i, j - 1, k), this.replaceBlock.defaultBlockState(), 2);
         this.host.onEat();
     }
}
