    ...
    private static Vector3d getTargetBlock(BaseCreatureEntity entity, int range, int height, Vector3d target, int heightLevel) {
        ...
        for(int attempt = 0; attempt < 10; ++attempt) {
            ...
            if(entity.isFlying() || (entity.isStrongSwimmer() && entity.isInWater())) {
	            if(entity.position().y() > entity.level().getHeightmapPos(Heightmap.Types.OCEAN_FLOOR, entity.blockPosition()).getY() + (heightLevel * 1.25))
	        		possibleY = random.nextInt(2 * height) - height * 3 / 2;
	            else if(entity.position().y() < entity.level().getHeightmapPos(Heightmap.Types.OCEAN_FLOOR, entity.blockPosition()).getY() + heightLevel)
