...
public class DefendVillageGoal extends FindAttackTargetGoal {
	private final TargetingConditions villageSearchPredicate = TargetingConditions.forCombat().range(64.0D);
	...
	public boolean canUse() {
		...
		List<Villager> villagers = this.host.level().getEntitiesOfClass(Villager.class, villageSearchArea, e -> this.villageSearchPredicate.test(null, this.host, e));
		List<Player> players = this.host.level().getEntitiesOfClass(Player.class, villageSearchArea, e -> this.villageSearchPredicate.test(null, this.host, e));
