...
public class StayByHomeGoal extends Goal {
    ...
    public boolean canUse() {
    	...
    	if(!this.host.hasHome() || this.host.getDistanceFromHome() <= 1.0F)
    		return false;
        if(!this.host.canBreatheUnderwaterCreature() && this.host.isInWater())
            return false;
    ...
    public void start() {
		...
		if(!host.useDirectNavigator())
			...
			host.setDirectNavigationTarget(new BlockPos(homePos.getX(), homePos.getY(), homePos.getZ()), speed);
    }
	...
	public void stop() {
		if(!this.host.useDirectNavigator())
			...
			this.host.clearDirectNavigationTarget(this.speed);
	}
}
