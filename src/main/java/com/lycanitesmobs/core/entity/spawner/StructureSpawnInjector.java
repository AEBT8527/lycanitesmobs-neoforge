...
public class StructureSpawnInjector extends JSONLoader {
    ...
    public void loadAllFromJson(ModInfo modInfo) {
        ...
        for (StructureSpawnConfig config : this.configs) {
            ...
            for (Identifier structureId : config.structures) {
    ...
    public void onServerAboutToStart(ServerAboutToStartEvent event) {
        ...
        for (StructureSpawnConfig config : this.configs) {
            ...
            if (oldSpawner != null && oldSpawner.isDefinitionEnabled()) {
                boolean hasStructureLocation = oldSpawner.hasLocationType(StructureSpawnLocation.class);
                ...
                LMHelperClass.logDebug("structurespawners", "No old spawner to disable for '" + config.name
                        + "' (spawner=" + (oldSpawner != null ? "exists,enabled=" + oldSpawner.isDefinitionEnabled() : "null") + ")");
            }
        }
        ...
        for (StructureSpawnConfig config : this.configs) {
            ...
            List<Weighted<MobSpawnSettings.SpawnerData>> spawnerDataList = config.buildSpawnerData();
            LMHelperClass.logDebug("structurespawners", "Config '" + config.name + "': buildSpawnerData returned " + spawnerDataList.size() + " entries.");
            for (Weighted<MobSpawnSettings.SpawnerData> lycWeighted : spawnerDataList) {
                MobSpawnSettings.SpawnerData data = lycWeighted.value();
                LMHelperClass.logDebug("structurespawners", "  SpawnerData: entityType=" + BuiltInRegistries.ENTITY_TYPE.getKey(data.type()) + " weight=" + lycWeighted.weight() + " min=" + data.minCount() + " max=" + data.maxCount());
            }
    ...
    private void injectSpawns(Structure structure, MobCategory category,
                              ...
                              List<Weighted<MobSpawnSettings.SpawnerData>> newSpawns,
        ...
        List<Weighted<MobSpawnSettings.SpawnerData>> mergedList;
        if (existing != null) {
        ...
        newOverrides.put(category, new StructureSpawnOverride(
                bbType, WeightedList.of(mergedList)));
        ...
        if (verifyOverride != null) {
            ...
            for (Weighted<MobSpawnSettings.SpawnerData> lycWeighted : verifyOverride.spawns().unwrap()) {
                MobSpawnSettings.SpawnerData data = lycWeighted.value();
                LMHelperClass.logDebug("structurespawners", "    [" + structureId + "]     -> " + BuiltInRegistries.ENTITY_TYPE.getKey(data.type()) + " w=" + lycWeighted.weight() + " min=" + data.minCount() + " max=" + data.maxCount());
            }
        } else {
    ...
    static class StructureSpawnConfig {
        ...
        void loadFromJSON(JsonObject json) {
            ...
            if (json.has("structures")) {
                ...
                for (JsonElement el : json.getAsJsonArray("structures")) {
                    this.structures.add(Identifier.parse(el.getAsString()));
                }
            }
            if (json.has("mobs")) {
                this.mobs.clear();
        ...
        List<Weighted<MobSpawnSettings.SpawnerData>> buildSpawnerData() {
            ...
            for (MobEntryConfig mob : this.mobs) {
                ...
                EntityType<?> entityType = BuiltInRegistries.ENTITY_TYPE.getValue(mob.mobId);
                if (entityType == null) {
                    LMHelperClass.logWarning("structurespawners",
                            "    !!! Entity type NOT FOUND in ForgeRegistries, skipping: " + mob.mobId);
                    continue;
                }
                Identifier resolvedId = BuiltInRegistries.ENTITY_TYPE.getKey(entityType);
                LMHelperClass.logDebug("structurespawners", "    Resolved to: " + resolvedId + " (entityType=" + entityType + ")");
                list.add(new Weighted<>(new MobSpawnSettings.SpawnerData(entityType, mob.minCount, mob.maxCount), mob.weight));
    ...
    static class MobEntryConfig {
        ...
        void loadFromJSON(JsonObject json) {
            if (json.has("mobId"))
                this.mobId = Identifier.parse(json.get("mobId").getAsString());
            if (json.has("weight"))
