...
public class WorldSpawnCondition extends SpawnCondition {
    ...
    public boolean isMet(Level world, Player player, BlockPos position) {
        ...
        long baseTime = worldExt != null ? worldExt.getConfiguredDayBaseTime(world) : LMHelperClass.getDayTime(world);

        int time = (int) (LMHelperClass.getDayTime(world) % 24000L);
        ...
        if (this.moonPhase >= 0 && (int) (LMHelperClass.getDayTime(world) / 24000L % 8L + 8L) % 8 != this.moonPhase) {
            return false;
        }
    ...
    public boolean isAllowedDimension(Level world) {
        return this.isAllowedDimensionId(world.dimension().identifier().toString());
    }
    ...
    public boolean isAllowedBiome(Level world, BlockPos position) {
        if (position != null) {
            ...
            if (biomeId == null) {
                LMHelperClass.logDebug("Dungeon", "WorldSpawnCondition biome check: biome Identifier is null at " + position);
    ...
    public void collectDebugFailures(Level world, Player player, BlockPos position, List<String> reasons) {
        ...
        if (this.moonPhase >= 0 && (int) (LMHelperClass.getDayTime(world) / 24000L % 8L + 8L) % 8 != this.moonPhase) {
            reasons.add("moonPhase " + (int) (LMHelperClass.getDayTime(world) / 24000L % 8L + 8L) % 8 + " != required " + this.moonPhase);
        }

        if (!this.isAllowedDimension(world)) {
            reasons.add("dimension " + world.dimension().identifier() + " disallowed (dimensionIds=" + this.dimensionIds + ", type=" + this.dimensionListType + ")");
        }
