...
public class RandomSpawnLocation extends BlockSpawnLocation {
    ...
    private BlockPos findSurfaceSpawnPos(Level world, int x, int z, int heightmapY) {
        ...
        if (heightmapY < world.getMinY() || heightmapY >= world.getMaxY()) {
            return null;
        }
    ...
    public int[] getRandomXZCoord(Level world, BlockPos triggerPos) {
        ...
        if (rangeMaxX * difficultyScale > 0) {
            xPos = world.getRandom().nextInt(rangeMaxX);
        ...
        if (rangeMaxZ * difficultyScale > 0) {
            zPos = world.getRandom().nextInt(rangeMaxZ);
            if (world.getRandom().nextBoolean()) {
                zPos += rangeMinZ;
            } else {
                zPos = -zPos - rangeMinZ;
    ...
    public int getRandomYCoord(Level world, BlockPos triggerPos) {
        ...
        int minY = Math.max(originY - rangeMaxY, world.getMinY());
        if (this.yMin >= 0) {
            minY = Math.max(minY, this.yMin);
        }
        int maxY = Math.min(originY + rangeMaxY, world.getMaxY() - 1);
        if (this.yMax >= 0) {
        ...
        for (int nextY = minY; nextY <= maxY; nextY++) {
            ...
            if (this.isValidBlock(world, spawnPos)) {
                ...
                if (nextY >= heightmapSurfaceY) {
                    if (!this.solidGround) {
                        ...
                        if (floatRange > 1) {
                            ...
                            nextY += world.getRandom().nextInt(floatRange + 1) - 1;
                            spawnPos.setY(nextY);
                            headPos.setY(nextY + 1);
                        }
                    }
                ...
                if (this.isValidHeadSpace(world, spawnPos, headPos)) {
                    if (nextY <= 64) {
                        countLow++;
                        if (world.getRandom().nextInt(countLow) == 0) reservoirLow = nextY;
                    } else {
                        countHigh++;
                        if (world.getRandom().nextInt(countHigh) == 0) reservoirHigh = nextY;
                    }
                }
        ...
        if (reservoirHigh != Integer.MIN_VALUE && (countLow == 0 || world.getRandom().nextFloat() > 0.25F)) {
    ...
    public boolean posHasGround(Level world, BlockPos pos) {
        if (pos == null || pos.getY() <= world.getMinY())
            return false;
