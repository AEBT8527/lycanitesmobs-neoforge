...
public class MobSpawn {
    ...
    public void resolveEntityType() {
        ...
        Identifier requestedId = Identifier.parse(this.mobId);
        EntityType<?> resolved = BuiltInRegistries.ENTITY_TYPE.getValue(requestedId);
        if (resolved != null) {
            Identifier resolvedKey = BuiltInRegistries.ENTITY_TYPE.getKey(resolved);

            if (resolvedKey != null && resolvedKey.equals(requestedId)) {
    ...
    public boolean canSpawn(Level world, int blockCount, Biome biome, boolean forceIgnoreDimension) {
        ...
        // Chance:
        if (this.getChance() < 1 && world.getRandom().nextDouble() > this.getChance()) {
            return false;
        }
    ...
    public LivingEntity createEntity(Level world) {
        try {
            ...
            Entity entity = this.entityType.create(world, net.minecraft.world.entity.EntitySpawnReason.MOB_SUMMONED);
    ...
    public void onSpawned(LivingEntity entityLiving, Player player) {
        ...
        if (entityLiving instanceof BaseCreatureEntity) {
            ...
            if (this.home >= 0) {
                entityCreature.setHome((int) entityCreature.position().x(), (int) entityCreature.position().y(),
                        (int) entityCreature.position().z(), (float) this.home);
            }
            if (this.mobLevel > 0) {
