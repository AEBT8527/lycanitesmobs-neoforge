...
public enum LycanitesMobType {
    ...
    public static LycanitesMobType of(LivingEntity entity) {
        ...
        if (type.builtInRegistryHolder().is(EntityTypeTags.UNDEAD)) {
            return UNDEAD;
        }
        if (type.builtInRegistryHolder().is(EntityTypeTags.ARTHROPOD)) {
            return ARTHROPOD;
        }
        if (type.builtInRegistryHolder().is(EntityTypeTags.ILLAGER)) {
            return ILLAGER;
        }
