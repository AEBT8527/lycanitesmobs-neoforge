...
public class PortalEntity extends BaseProjectileEntity {
    ...
    // Datawatcher:
    protected static final EntityDataAccessor<Optional<net.minecraft.world.entity.EntityReference<net.minecraft.world.entity.LivingEntity>>> OWNER_UUID = SynchedEntityData.defineId(PortalEntity.class, EntityDataSerializers.OPTIONAL_LIVING_ENTITY_REFERENCE);
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide()) {
            if (this.shootingEntity != null) {
                this.entityData.set(OWNER_UUID, Optional.of(net.minecraft.world.entity.EntityReference.of(this.shootingEntity.getUUID())));
            } else if (this.summoningPedestal != null && this.summoningPedestal.getOwnerUUID() != null) {
                this.entityData.set(OWNER_UUID, Optional.of(net.minecraft.world.entity.EntityReference.of(this.summoningPedestal.getOwnerUUID())));
            } else {
                this.entityData.set(OWNER_UUID, Optional.empty());
            }
        } else {
            this.ownerUUID = this.entityData.get(OWNER_UUID).map(net.minecraft.world.entity.EntityReference::getUUID).orElse(null);
        }
        ...
        if (this.level().isClientSide()) {
            for (int i = 0; i < 32; ++i) {
                double angle = Math.toRadians(this.getRandom().nextFloat() * 360);
                float distance = this.getRandom().nextFloat() * 2;
                double x = distance * Math.cos(angle) + Math.sin(angle);
                double z = distance * Math.sin(angle) - Math.cos(angle);
                this.level().addParticle(ParticleTypes.PORTAL,
                        this.position().x() + x,
                        this.position().y() + (4.0F * this.getRandom().nextFloat()) - 2.0F,
                        this.position().z() + z,
                        0.0D, 0.0D, 0.0D);
            }
            return;
    ...
    public int summonCreatures() {
        ...
        for (int i = 0; i < totalSummonAmount; i++) {
            Entity entity = this.summonType.create(this.level(), net.minecraft.world.entity.EntitySpawnReason.MOB_SUMMONED);
            if (entity == null) {
                return 0;
            }
            entity.snapTo(this.position().x(), this.position().y(), this.position().z(), this.getRandom().nextFloat() * 360.0F, 0.0F);
            ...
            if (this.isAlive()) {
                DeferredLevelActionManager.spawnEntity(this.level(), this.blockPosition(), "portal_summon:" + entity.getUUID(), entity);
            }
        }
    ...
    public Identifier getTexture() {
        Identifier baseTexture = AssetHelper.texture("textures/particles/" + this.entityName.toLowerCase() + ".png");
        Identifier clientTexture = AssetHelper.texture("textures/particles/" + this.entityName.toLowerCase() + "_client.png");
        Identifier playerTexture = AssetHelper.texture("textures/particles/" + this.entityName.toLowerCase() + "_player.png");
