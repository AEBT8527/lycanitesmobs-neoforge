...
public abstract class AgeableCreatureEntity extends BaseCreatureEntity {
    ...
    public void setupMob() {
        if (this.babySpawnChance > 0D && this.getRandom().nextDouble() < this.babySpawnChance)
    ...
    public boolean performCommand(String command, Player player, ItemStack itemStack, InteractionHand hand) {
        ...
        // Spawn Baby:
        if (command.equals("Spawn Baby") && !this.level().isClientSide() && itemStack.getItem() instanceof ItemCustomSpawnEgg) {
            ...
            if (spawnEggCreatureInfo != null) {
                if (spawnEggCreatureInfo.matchesEntityClass(this.getClass())) {
                    ...
                    if (baby != null) {
                        ...
                        baby.snapTo(this.position().x(), this.position().y(), this.position().z(), 0.0F, 0.0F);
                        baby.setFarmed();
                        DeferredLevelActionManager.spawnEntity(this.level(), this.blockPosition(), null, baby, () -> {
    ...
    public AgeableCreatureEntity createChild(AgeableCreatureEntity partner) {
        return (AgeableCreatureEntity) this.creatureInfo.createEntity(this.level());
    }
    ...
    public void procreate(AgeableCreatureEntity partner) {
        ...
        if (baby != null) {
            ...
            baby.snapTo(this.position().x(), this.position().y(), this.position().z(), this.yRotO, this.xRotO);

            for (int i = 0; i < 7; ++i) {
                double d0 = this.getRandom().nextGaussian() * 0.02D;
                double d1 = this.getRandom().nextGaussian() * 0.02D;
                double d2 = this.getRandom().nextGaussian() * 0.02D;
                this.level().addParticle(ParticleTypes.HEART, this.position().x() + (double) (this.getRandom().nextFloat() * this.getDimensions(Pose.STANDING).width() * 2.0F) - (double) this.getDimensions(Pose.STANDING).width(), this.position().y() + 0.5D + (double) (this.getRandom().nextFloat() * this.getDimensions(Pose.STANDING).height()), this.position().z() + (double) (this.getRandom().nextFloat() * this.getDimensions(Pose.STANDING).width() * 2.0F) - (double) this.getDimensions(Pose.STANDING).width(), d0, d1, d2);
            }
    ...
    public void readAdditionalSaveData(CompoundTag nbt) {
        ...
        if (nbt.contains("Age")) {
            this.setGrowingAge(nbt.getIntOr("Age", 0));
        } else {
            this.setGrowingAge(0);
        }

        if (nbt.contains("InLove")) {
            this.loveTime = nbt.getIntOr("InLove", 0);
        } else {
            this.loveTime = 0;
        }

        if (nbt.contains("HasBeenFarmed")) {
            if (nbt.getBooleanOr("HasBeenFarmed", false)) {
                this.setFarmed();
            }
        }
    }
