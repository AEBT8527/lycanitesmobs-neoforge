...
public abstract class RideableCreatureEntity extends TameableCreatureEntity {
    ...
    public void aiStep() {
		...
		if(this.hasRiderTarget()) {
			...
			// Mount Melee:
			if(!this.level().isClientSide() && this.hasAttackTarget() && this.updateTick % 20 == 0) {
...
//			}
    ...
    public void riderEffects(LivingEntity rider) {
        if(!rider.canBreatheUnderwater() && this.canBreatheUnderwaterCreature() && rider.isInWater())
    ...
    public void positionRider(Entity passenger,MoveFunction moveFunction) {
        if(this.hasPassenger(passenger)) {
			...
			Vector3d mountOffset = this.getFacingPositionDouble(0, 0, 0, zOffset, this.yRotO);
            this.getControllingPassenger().setPos(this.position().x() + mountOffset.x, this.position().y() + this.getPassengersRidingOffset(), this.position().z() + mountOffset.z);
        }
    }

    private void mount(Entity entity) {
    	entity.yRotO = this.yRotO;
    	entity.xRot = this.xRotO;
        if(!this.level().isClientSide())
    ...
    public void moveMountedWithHeading(double strafe, double up, double forward) {
        ...
        if(this.getControllingPassenger() instanceof LivingEntity) {
            ...
            this.yRotO = this.yRot = rider.getYRot();
        ...
        if(this.isInWater() || this.isInLava() || this.isFlying()) {
            ...
            if (this.getJumpPower() > 0.0F && !this.isMountJumping() && this.isControlledByLocalInstance()) {
                ...
                if (this.hasEffect(MobEffects.JUMP_BOOST))
					this.setDeltaMovement(this.getDeltaMovement().add(0, ((float) (this.getEffect(MobEffects.JUMP_BOOST).getAmplifier() + 1) * 0.1F), 0));
            ...
            this.setFlyingSpeed((float) (this.getSpeed() * this.getGlideScale()));
        }

		// Ability Controls:
		if(this.getControllingPassenger() instanceof Player player) {
			this.handleRiderControls(player, ExtendedPlayer.getForPlayer(player));
		}
        ...
        if(this.isControlledByLocalInstance()) {
            ...
            if(!this.useDirectNavigator()) {
                if(this.isFlying() && !this.isInWater() && !this.isInLava()) {
					...
					if(!this.canBreatheUnderwaterCreature()) {
    ...
    public HashMap<Integer, String> getInteractCommands(Player player, @Nonnull ItemStack itemStack) {
    	...
    	if(this.canBeMounted(player) && !player.isShiftKeyDown() && !this.level().isClientSide() && mountingAllowed)
	...
	// ==================================================
  	...
  	// ==================================================
    @Override
    public boolean hurtServer(net.minecraft.server.level.ServerLevel lycLevel, DamageSource damageSource, float damageAmount) {
        Entity entity = damageSource.getEntity();
        return this.getControllingPassenger() != null && this.isEntityPassenger(entity, this) ? false : super.hurtServer(lycLevel, damageSource, damageAmount);
    }
