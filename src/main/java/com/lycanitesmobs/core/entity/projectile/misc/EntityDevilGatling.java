...
public class EntityDevilGatling extends BaseProjectileEntity {
    ...
    public void tick() {
        ...
        if (this.position().y() > this.level().getMaxY() + 20)
        ...
        if (this.getRandom().nextBoolean()) {
            ...
            for (Object potionEffectObj : target.getActiveEffects()) {
                if (potionEffectObj instanceof MobEffectInstance) {
                    MobEffect effect = ((MobEffectInstance) potionEffectObj).getEffect().value();
                    if (effect != null) {
            ...
            if (!goodEffects.isEmpty()) {
                if (goodEffects.size() > 1)
                    target.removeEffect(ObjectManager.holder(goodEffects.get(this.getRandom().nextInt(goodEffects.size()))));
                else
                    target.removeEffect(ObjectManager.holder(goodEffects.get(0)));
            }
        }

        if (ObjectManager.getEffect("decay") != null) {
            target.addEffect(new MobEffectInstance(ObjectManager.holder(ObjectManager.getEffect("decay")), this.getEffectDuration(60), 0));
        }
    }
    ...
    public void onImpactVisuals() {
        for (int i = 0; i < 8; ++i)
            this.level().addParticle(ParticleTypes.EXPLOSION, this.position().x(), this.position().y(), this.position().z(), 0.0D, 0.0D, 0.0D);
    }
