...
public class EntityHellfireWave extends BaseProjectileEntity {
            ...
            for (int row = 0; row < this.hellfireHeight; row++) {
                for (int col = 0; col < this.hellfireWidth; col++) {
                    if (this.getOwner() != null)
                        this.hellfireWalls[row][col] = new EntityHellfireWavePart(ProjectileManager.getInstance().getOldProjectileType(EntityHellfireWavePart.class), this.level(), (LivingEntity) this.getShooter());
                    else
                        this.hellfireWalls[row][col] = new EntityHellfireWavePart(ProjectileManager.getInstance().getOldProjectileType(EntityHellfireWavePart.class), this.level(), this.position().x(), this.position().y() + 5 + (this.hellfireSize * row), this.position().z());
    ...
    public boolean onEntityLivingDamage(LivingEntity entityLiving) {
        if (!com.lycanitesmobs.core.util.helpers.LMHelperClass.isInvulnerableTo(entityLiving, entityLiving.level().damageSources().inFire()))
