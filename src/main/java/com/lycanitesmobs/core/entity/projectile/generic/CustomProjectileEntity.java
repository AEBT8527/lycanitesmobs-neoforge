...
public class CustomProjectileEntity extends BaseProjectileEntity {
    ...
    public void setProjectileInfo(ProjectileInfo projectileInfo) {
        ...
        if (this.rollSpeed > 0 && this.getRandom().nextBoolean()) {
    ...
    public void tick() {
        ...
        if (this.projectileInfo != null) {
            ...
            // Particles:
            if (this.level().isClientSide() && this.projectileInfo.getParticleCount() > 0) {
                for (int i = 0; i < this.projectileInfo.getParticleCount(); ++i) {
                    if (this.isInWater()) {
                        if (this.projectileInfo.getWaterParticleType() != null) {
                            this.level().addParticle(this.projectileInfo.getWaterParticleType().getType(), this.getX() + (this.getRandom().nextDouble() - 0.5D) * (double) this.getBbWidth(), this.getY() + this.getRandom().nextDouble() * (double) this.getBbHeight(), this.getZ() + (this.getRandom().nextDouble() - 0.5D) * (double) this.getBbWidth(), 0.0D, 0.0D, 0.0D);
                        } else if ("minecraft:dust_redstone".equalsIgnoreCase(this.projectileInfo.getWaterParticleId())) {
                            this.level().addParticle(DustParticleOptions.REDSTONE, this.getX() + (this.getRandom().nextDouble() - 0.5D) * (double) this.getBbWidth(), this.getY() + this.getRandom().nextDouble() * (double) this.getBbHeight(), this.getZ() + (this.getRandom().nextDouble() - 0.5D) * (double) this.getBbWidth(), 0.0D, 0.0D, 0.0D);
                        } else if ("minecraft:dust_falling_dirt".equalsIgnoreCase(this.projectileInfo.getWaterParticleId())) {
                            this.level().addParticle(new BlockParticleOption(ParticleTypes.BLOCK, Blocks.DIRT.defaultBlockState()), this.getX() + (this.getRandom().nextDouble() - 0.5D) * (double) this.getBbWidth(), this.getY() + this.getRandom().nextDouble() * (double) this.getBbHeight(), this.getZ() + (this.getRandom().nextDouble() - 0.5D) * (double) this.getBbWidth(), 0.0D, 0.0D, 0.0D);
                        }
                    } else {
                        if (this.projectileInfo.getParticleType() != null) {
                            this.level().addParticle(this.projectileInfo.getParticleType().getType(), this.getX() + (this.getRandom().nextDouble() - 0.5D) * (double) this.getBbWidth(), this.getY() + this.getRandom().nextDouble() * (double) this.getBbHeight(), this.getZ() + (this.getRandom().nextDouble() - 0.5D) * (double) this.getBbWidth(), 0.0D, 0.0D, 0.0D);
    ...
    public boolean canDamage(LivingEntity targetEntity) {
        ...
        if (this.projectileInfo != null) {
            canDamage = this.projectileInfo.canDamage(this, this.level(), targetEntity, canDamage);
        }
        return canDamage;
    }
    ...
    public void syncThrower() {
        if (!this.level().isClientSide()) {
            ...
            if (this.throwerId == -1) {
                ...
                Entity possibleThrower = this.level().getEntity(this.throwerId);
    ...
    public void onDamage(LivingEntity target, float damage, boolean attackSuccess) {
        ...
        if (!this.level().isClientSide() && attackSuccess && this.projectileInfo != null) {
            if (this.projectileInfo.getDebuffsOverride() != null) {
                ...
                for (String debuff : this.projectileInfo.getDebuffsOverride()) {
                    MobEffect effect = BuiltInRegistries.MOB_EFFECT.getValue(Identifier.parse(debuff));
                    if (effect != null) {
        ...
        if (attackSuccess && this.projectileInfo != null) {
            this.projectileInfo.onProjectileDamage(this, this.level(), target, damage);
        }
    }
    ...
    public void onImpactComplete(BlockPos impactPos) {
        ...
        this.projectileInfo.onProjectileImpact(this, this.level(), impactPos);
    }
    ...
    public BaseProjectileEntity getParent() {
        if (this.level().isClientSide()) {
            ...
            if (this.parentId == -1) {
                ...
                Entity possibleParent = this.level().getEntity(this.parentId);
    ...
    public Entity getTarget() {
        if (this.level().isClientSide()) {
            ...
            if (this.targetId == -1) {
                ...
                this.target = this.level().getEntity(this.targetId);
            }
        }
        return this.target;
    }
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        ...
        if (compound.contains("ProjectileName")) {
            this.loadProjectileInfo(compound.getStringOr("ProjectileName", ""));
        }
    }
