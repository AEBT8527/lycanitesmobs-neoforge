...
public class LaserProjectileEntity extends BaseProjectileEntity {
    ...
    public void defineSynchedData(net.minecraft.network.syncher.SynchedEntityData.Builder builder) {
        ...
        builder.define(SHOOTING_ENTITY_ID, -1);
        builder.define(LASER_END_ID, -1);
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide() && this.shootingEntity != null) {
    ...
    public void updateEnd() {
        if (this.level().isClientSide()) {
            ...
            if (this.laserEndRef != -1)
                possibleLaserEnd = this.level().getEntity(this.laserEndRef);
        ...
        if (this.laserEnd == null)
            ...
            HitResult rayTraceResult = LMHelperClass.raytrace(this.level(), this.position().x(), this.position().y(), this.position().z(), this.targetX, this.targetY, this.targetZ, this.laserWidth, this, excludedEntities);
        ...
        if (this.getBeamSound() != null)
            this.playSound(this.getBeamSound(), 1.0F, 1.0F / (this.getRandom().nextFloat() * 0.4F + 0.8F));
    }
    ...
    public void syncShootingEntity() {
        if (!this.level().isClientSide()) {
            ...
            if (this.shootingEntityRef == -1) this.shootingEntity = null;
                ...
                Entity possibleShootingEntity = this.level().getEntity(this.shootingEntityRef);
    ...
    public void readAdditionalSaveData(CompoundTag nbtTagCompound) {
        if (nbtTagCompound.contains("LaserTime"))
            this.setTime(nbtTagCompound.getIntOr("LaserTime", 0));
        if (nbtTagCompound.contains("OffsetX"))
            this.offsetX = nbtTagCompound.getDoubleOr("OffsetX", 0.0D);
        if (nbtTagCompound.contains("OffsetY"))
            this.offsetY = nbtTagCompound.getDoubleOr("OffsetY", 0.0D);
        if (nbtTagCompound.contains("OffsetZ"))
            this.offsetZ = nbtTagCompound.getDoubleOr("OffsetZ", 0.0D);
