...
public class PetEntry {
    ...
    public PetEntry setSpawningActive(boolean spawningActive) {
        ...
        if (!this.host.level().isClientSide()) {
            if (!CreatureManager.getInstance().getConfig().isSoulboundAllowed(this.host.level())) {
    ...
    public void spawnEntity() {
        if (!(this.host != null && this.host.level() instanceof ServerLevel serverLevel)) {
            return;
        }

        DeferredLevelActionManager.enqueue(serverLevel, this.host.blockPosition(), "pet_spawn:" + this.petEntryID, level -> {
            if (this.entity != null || this.host == null || this.host.level() != level || !this.active || !this.spawningActive) {
                return;
            }
            this.spawnEntityNow();
    ...
    private void spawnEntityNow() {
        ...
        try {
            this.entity = this.summonSet.getCreatureType().create(this.host.level(), net.minecraft.world.entity.EntitySpawnReason.MOB_SUMMONED);
        }
        catch (Exception e) {
        ...
        this.loadEntityNBT();
        this.entity.snapTo(this.host.position().x(), this.host.position().y(), this.host.position().z(), this.host.yRotO, 0.0F);
        ...
        DeferredLevelActionManager.spawnEntityNow(this.host.level(), this.entity, () -> {
    ...
    private void moveEntityToValidHostOffset(BaseCreatureEntity entityCreature) {
        ...
        this.tryMoveEntityToHostOffset(entityCreature, -randomAngle);
    }
    ...
    private boolean tryMoveEntityToHostOffset(BaseCreatureEntity entityCreature, float angle) {
        ...
        if (!this.entity.level().getBlockState(spawnPos).isValidSpawn(this.entity.level(), spawnPos, this.entity.getType())) {
            return false;
        }
        this.entity.snapTo(spawnPos.getX(), spawnPos.getY(), spawnPos.getZ(), this.host.yRotO, 0.0F);
        return true;
    }
    ...
    private void teleportEntityToHost() {
        if (this.entity.level() != this.host.level()) {
            if (!(this.host.level() instanceof net.minecraft.server.level.ServerLevel hostLevel)) {
                return;
            }
            Entity moved = this.entity.teleport(new net.minecraft.world.level.portal.TeleportTransition(
                    hostLevel,
                    this.host.position(), net.minecraft.world.phys.Vec3.ZERO,
    ...
    public void readFromNBT(CompoundTag nbtTagCompound) {
        if (nbtTagCompound.read("UUID", net.minecraft.core.UUIDUtil.CODEC).isPresent())
            this.petEntryID = nbtTagCompound.read("UUID", net.minecraft.core.UUIDUtil.CODEC).orElse(null);
        if (nbtTagCompound.contains("Type"))
            this.type = nbtTagCompound.getStringOr("Type", "");

        if (nbtTagCompound.contains("Active"))
            this.active = nbtTagCompound.getBooleanOr("Active", false);
        if (nbtTagCompound.contains("RespawnTime"))
            this.respawnTime = nbtTagCompound.getIntOr("RespawnTime", 0);
        if (nbtTagCompound.contains("Respawning"))
            this.isRespawning = nbtTagCompound.getBooleanOr("Respawning", false);
        if (nbtTagCompound.contains("SpawningActive"))
            this.spawningActive = nbtTagCompound.getBooleanOr("SpawningActive", false);

        this.summonSet.read(nbtTagCompound);

        if (nbtTagCompound.contains("EntityName"))
            this.setEntityName(nbtTagCompound.getStringOr("EntityName", ""));
        if (!"familiar".equals(this.getType())) {
            this.readCreatureAppearance(nbtTagCompound);
        }
        if (nbtTagCompound.contains("EntityNBT"))
            this.entityNBT = nbtTagCompound.getCompoundOrEmpty("EntityNBT");
        this.loadEntityNBT();
    }
    ...
    public void writeToNBT(CompoundTag nbtTagCompound) {
        nbtTagCompound.store("UUID", net.minecraft.core.UUIDUtil.CODEC, this.petEntryID);
    ...
    public void saveEntityNBT() {
        ...
        if (this.entity instanceof BaseCreatureEntity baseCreatureEntity) {
            ...
            try {
                net.minecraft.world.level.storage.TagValueOutput lycOut = net.minecraft.world.level.storage.TagValueOutput.createWithContext(net.minecraft.util.ProblemReporter.DISCARDING, this.entity.registryAccess());
                this.entity.saveWithoutId(lycOut);
                this.entityNBT = lycOut.buildResult();
            } catch (Throwable t) {
    ...
    public void loadEntityNBT() {
        ...
        if (this.entityNBT.contains("MobLevel")) {
            this.setLevel(this.entityNBT.getIntOr("MobLevel", 0));
        }
        if (this.entityNBT.contains("Experience")) {
            this.setExperience(this.entityNBT.getIntOr("Experience", 0));
        }
        ...
        if (this.entity instanceof BaseCreatureEntity baseCreatureEntity) {
            ...
            if (this.entity instanceof AgeableCreatureEntity ageableCreatureEntity) {
                if (this.entityNBT.contains("Age"))
                    ageableCreatureEntity.setGrowingAge(this.entityNBT.getIntOr("Age", 0));
                else
    ...
    private void readCreatureAppearance(CompoundTag nbtTagCompound) {
        if (nbtTagCompound.contains("SubspeciesID")) {
            this.setEntitySubspecies(Subspecies.getIndexFromOld(nbtTagCompound.getIntOr("SubspeciesID", 0)));
            this.setEntityVariant(Variant.getIndexFromOld(nbtTagCompound.getIntOr("SubspeciesID", 0)));
        }
        if (nbtTagCompound.contains("Subspecies"))
            this.setEntitySubspecies(nbtTagCompound.getIntOr("Subspecies", 0));
        if (nbtTagCompound.contains("Variant"))
            this.setEntityVariant(nbtTagCompound.getIntOr("Variant", 0));
        if (nbtTagCompound.contains("EntitySize"))
            this.setEntitySize(nbtTagCompound.getDoubleOr("EntitySize", 0.0D));
        if (nbtTagCompound.contains("Color"))
            this.setColor(nbtTagCompound.getStringOr("Color", ""));
    }
}
