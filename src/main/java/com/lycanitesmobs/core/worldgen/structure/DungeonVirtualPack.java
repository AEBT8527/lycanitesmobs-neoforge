...
public class DungeonVirtualPack implements PackResources {
    ...
    public DungeonVirtualPack(String packId) {
        ...
        this.locationInfo = new net.minecraft.server.packs.PackLocationInfo(
                packId, net.minecraft.network.chat.Component.literal(packId),
                net.minecraft.server.packs.repository.PackSource.BUILT_IN, java.util.Optional.empty());
        generateResources();
    }

    public DungeonVirtualPack(net.minecraft.server.packs.PackLocationInfo location) {
        this.packId = location.id();
    ...
    private void generateResources() {
        try {
            ...
            Path jarSchematicsDir = null;
            for (Path lycRoot : modFile.getContents().getContentRoots()) {
                Path lycCandidate = lycRoot.resolve("common").resolve("lycanitesmobs").resolve("dungeons").resolve("schematics");
                if (Files.isDirectory(lycCandidate)) {
                    jarSchematicsDir = lycCandidate;
                    break;
                }
            }
    ...
    private void generateBiomeTag(String name, List<String> biomeTags, List<String> biomeIds) {
        ...
        resources.put(
                Identifier.fromNamespaceAndPath("lycanitesmobs", "tags/worldgen/biome/has_structure/" + name + ".json"),
                toBytes(tag)
        );
    }
    ...
    private void generateStructure(String name) {
        ...
        resources.put(
                Identifier.fromNamespaceAndPath("lycanitesmobs", "worldgen/structure/" + name + ".json"),
                toBytes(structure)
        );
    }
    ...
    private void generateCombinedStructureSet(List<SchematicEntry> entries, int spacing, int separation, int salt) {
        ...
        resources.put(
                Identifier.fromNamespaceAndPath("lycanitesmobs", "worldgen/structure_set/" + COMBINED_SET_NAME + ".json"),
                toBytes(set)
        );
    }
    ...
    public <T> T getMetadataSection(MetadataSectionType<T> deserializer) throws IOException {
        ...
        try (InputStream in = root.get()) {
            ...
            String sectionName = deserializer.name();
            if (meta.has(sectionName)) {
                return deserializer.codec()
                        .parse(com.mojang.serialization.JsonOps.INSTANCE, meta.get(sectionName))
                        .result().orElse(null);
            }
        }
        return null;
