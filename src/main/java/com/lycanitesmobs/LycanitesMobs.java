...
public class LycanitesMobs {
    ...
    // Proxy:
    public static IProxy PROXY = FMLEnvironment.getDist() == Dist.CLIENT ? new ClientProxy() : new ServerProxy();
    ...
    public LycanitesMobs(IEventBus modEventBus, ModContainer modContainer) {
        ...
        if (FMLEnvironment.getDist() == Dist.CLIENT) {
            modEventBus.addListener(com.lycanitesmobs.client.ClientFluidModels::onRegisterFluidModels);
            modEventBus.addListener(com.lycanitesmobs.client.ClientFluidModels::onRegisterSpecialModelRenderers);
            modEventBus.addListener(com.lycanitesmobs.client.ClientFluidModels::onRegisterItemTintSources);
            try {
    ...
    private void commonSetup(final FMLCommonSetupEvent event) {
        // Vanilla Item Lists — 26.x: ItemStack creation needs bound data components, so this
        // cannot run during mod construction anymore.
        ObjectLists.createVanillaLists();
        net.neoforged.neoforge.common.NeoForge.EVENT_BUS.addListener(
                (net.neoforged.neoforge.event.server.ServerAboutToStartEvent lycEvent) -> ObjectLists.lycFlushPendingItems());
