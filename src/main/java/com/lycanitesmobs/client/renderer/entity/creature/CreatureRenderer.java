...
public class CreatureRenderer<T extends BaseCreatureEntity> extends MobRenderer<T, CreatureModel<T>> {
    ...
    public void render(T entity, float partialTicks, float yaw, PoseStack matrixStack, MultiBufferSource renderTypeBuffer, int brightness) {
        if (NeoForge.EVENT_BUS.post(new RenderLivingEvent.Pre<>(entity, this, partialTicks, matrixStack, renderTypeBuffer, brightness)).isCanceled()) {
            return;
        }
        ...
        try {
            this.layers.clear();
            this.model = ModelManager.getInstance().getCreatureModel(entity.getCreatureInfo(), entity.getSubspecies());
            if (this.model == null) {
                return;
            }
        ...
        try {
            ...
            this.setupRotations(entity, matrixStack, loop, renderYaw, yaw, 1.0F);
        ...
        // Render Name Tag
        if (!invisible) {
            RenderNameTagEvent renderNameplateEvent = new RenderNameTagEvent(entity, entity.getDisplayName(), this, matrixStack, renderTypeBuffer, brightness, yaw);
            NeoForge.EVENT_BUS.post(renderNameplateEvent);
            if (renderNameplateEvent.canRender().isTrue() || (renderNameplateEvent.canRender() != net.neoforged.neoforge.common.util.TriState.FALSE && this.shouldShowName(entity))) {
                MutableComponent entityName = entity.getName().copy();
                if (entityName != null) {
                    this.renderNameTag(entity, entityName, matrixStack, renderTypeBuffer, brightness, yaw);
                }
            }
        }

        NeoForge.EVENT_BUS.post(new RenderLivingEvent.Post<>(entity, this, partialTicks, matrixStack, renderTypeBuffer, brightness));
    }
    ...
    protected void renderModel(BaseCreatureEntity entity, PoseStack matrixStack, MultiBufferSource renderTypeBuffer, LayerCreatureBase layer, float time, float distance, float loop, float lookY, float lookX, float scale, int brightness, int fade, boolean invisible, boolean allyInvisible) {
        ...
        if (OculusCompat.isShaderPackActive()) {
            if (!invisible || allyInvisible) {
                ...
                VBOObjModel.useEntityFormat = true;
                VBOObjModel.irisRenderType = OculusCompat.getIrisEntityRenderType(texture, blending, glow);
                VBOObjModel.irisBlending = blending;
                VBOObjModel.irisGlow = glow;
                ...
                try {
                    ...
                    VBOObjModel.useEntityFormat = false;
                    VBOObjModel.irisRenderType = null;
                    VBOObjModel.irisBlending = 0;
                    VBOObjModel.irisGlow = false;
                }
            }
            return;
        }
    ...
    protected float getBob(BaseCreatureEntity creatureEntity, float particalTicks) {
        if (!Minecraft.getInstance().isPaused()) {
            creatureEntity.advanceRenderTick(Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true));
        }
