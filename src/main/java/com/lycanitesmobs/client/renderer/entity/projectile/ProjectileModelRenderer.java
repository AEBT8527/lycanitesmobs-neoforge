...
public class ProjectileModelRenderer extends EntityRenderer<BaseProjectileEntity, ProjectileRenderState> {
    ...
    public ProjectileModelRenderer(EntityRendererProvider.Context renderManager, String projectileName) {
        super(renderManager);
    ...
    protected void renderLegacy(BaseProjectileEntity entity, float partialTicks, float yaw, PoseStack matrixStack, MultiBufferSource renderTypeBuffer, int brightness) {
        ...
        try {
            ...
            if (this.getModel() == null) {
                ...
                for (LayerProjectileBase layerCreatureBase : this.renderLayers) {
    ...
    protected void renderModel(BaseProjectileEntity entity, PoseStack matrixStack, MultiBufferSource renderTypeBuffer, LayerProjectileBase layer, float time, float distance, float loop, float lookY, float lookX, float scale, int brightness, boolean invisible, boolean allyInvisible) {
        ...
        // Same path CreatureRenderer uses. This used to set the VBOObjModel statics and pass a null
        // VertexConsumer, which was the old GL batching fast path - that path is gone on 26.x, the
        // statics are inert and ObjModel.renderPart returns immediately on a null consumer, so every
        // obj-model projectile (lightball, chaosorb, aetherwave, crystalshard, lobdarklings) drew
        // nothing at all.
    ...
    public final boolean addLayer(LayerProjectileBase layer) {
