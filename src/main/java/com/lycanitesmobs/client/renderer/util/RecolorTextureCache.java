...
public final class RecolorTextureCache {
    ...
    private static final Map<RecolorKey, Identifier> cache = new ConcurrentHashMap<>();

    /**
     * Returns a {@link Identifier} pointing to a texture with the
     ...
     */
    public static Identifier getRecoloredTexture(
            Identifier original,
        ...
        Identifier recolored = buildRecoloredTexture(original, baseR, baseG, baseB, baseRange, varR, varG, varB, varAmount);
     ...
     */
    private static Identifier buildRecoloredTexture(
            Identifier original,
            float baseR, float baseG, float baseB, float baseRange,
            float varR, float varG, float varB, float varAmount) {
        try {
            ...
            if (resource.isEmpty()) {
                LMHelperClass.logWarningMessage("[Recolor] Texture not found: " + original);
                return null;
            }
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    ...
                    if (blend > 0.001F) {
                        ...
                        image.setPixel(x, y, ri | (gi << 8) | (bi << 16) | (a << 24));
                    }
                }
            }

            DynamicTexture dynamicTexture = new DynamicTexture(() -> "lycanitesmobs recolor", image);
            ...
            Identifier recoloredLoc = Identifier.fromNamespaceAndPath(original.getNamespace(), uniqueName);

            Minecraft.getInstance().getTextureManager().register(recoloredLoc, dynamicTexture);

            LMHelperClass.logDebug("Resources", "Recolored texture cached: " + recoloredLoc);
            return recoloredLoc;

        } catch (Exception e) {
            LMHelperClass.logWarningMessage("[Recolor] Failed to recolor texture: " + original + " - " + e.getMessage());
            return null;
        }
    }
}
