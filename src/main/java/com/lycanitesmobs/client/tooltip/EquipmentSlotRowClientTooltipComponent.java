...
public class EquipmentSlotRowClientTooltipComponent implements ClientTooltipComponent {
    ...
    private static final Identifier GUI_ICONS =
            Identifier.fromNamespaceAndPath("minecraft", "textures/gui/icons.png");
    ...
    public void extractImage(Font font, int x, int y, int lycW, int lycH, GuiGraphicsExtractor graphics) {
        ...
        graphics.textRenderer().accept(levelTitleX, levelTitleY, levelTitle.copy().withColor(0xFFD700));
        ...
        if (data.getLevel() < data.getLevelMax()) {
            ...
            pose.pushMatrix();
            pose.translate(xpBarLeft, xpBarTop);
            pose.scale(scale, 1F);
            graphics.fill(0, 0, xpBarTexWidth, xpBarTexHeight, 0xAA202020);

            int filledTexWidth = (int) (xpBarTexWidth * ratio);
            if (filledTexWidth > 0) {
                graphics.fill(0, 0, filledTexWidth, xpBarTexHeight, 0xFF57C447);
            }
