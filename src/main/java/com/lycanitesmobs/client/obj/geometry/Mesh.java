    ...
    public VertexBuffer getVbo() {
        try {
            if (this.vbo == null) {
                ...
                BufferBuilder buf = tess.begin(VertexFormat.Mode.TRIANGLES, CustomRenderStates.POS_TEX_NORMAL);
                ...
                for (int i = 0; i < this.indices.length; i++) {
                    ...
                    if (normal == null) {
                        ...
                        if (!logs.contains(message)) {
                            LMHelperClass.logWarningMessage( message);
                            logs.add(message);
                        }
                        normal = new Vector3o(0, 0, 0);
                    }

                    buf.addVertex(px, py, pz)
                            .setUv(u, v)
                            .setNormal(normal.x(), normal.y(), normal.z())
                            ;
                }

                // Some OBJ parts legitimately end up with zero faces (e.g. material-filtered or
                // placeholder groups); 1.21's buildOrThrow crashes on an empty builder, so skip.
                com.mojang.blaze3d.vertex.MeshData meshData = buf.build();
                if (meshData == null) {
                    return null;
                }
