...
public class ObjModel {
    ...
    public ObjModel(Identifier resourceLocation, ResourceManager resourceManager) {
    ...
    private void initFromResource(Identifier resourceLocation, ResourceManager resourceManager) {
    ...
    public void renderPart(VertexConsumer vertexBuilder, Matrix3f matrix3f, Matrix4f matrix4f, int brightness, int fade, ObjPart objPart, Vector4f color, Vector2f textureOffset) {
        ...
        // Vanilla entity render types assemble QUADS; OBJ faces are triangles, so emit the
        // third vertex twice to form a degenerate quad per triangle.
        ...
        for (int i = 0; i < indices.length; i += 3) {
            ...
            for (int iv : lycTriToQuad) {
                ...
                org.joml.Vector3f transformedNormal = matrix3f.transform(new org.joml.Vector3f(normal.x(), normal.y(), normal.z()));
                vertexBuilder
                        .addVertex(matrix4f, v.getPos().x(), v.getPos().y(), v.getPos().z())
                        .setColor(color.x(), color.y(), color.z(), color.w())
                        .setUv(v.getTexCoords().x + (textureOffset.x * 0.01f), 1f - (v.getTexCoords().y + (textureOffset.y * 0.01f)))
                        .setUv1(0, 10 - fade)
                        .setLight(brightness)
