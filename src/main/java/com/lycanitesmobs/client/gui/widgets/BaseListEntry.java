...
public abstract class BaseListEntry extends AbstractSelectionList.Entry<BaseListEntry> {
	...
	public void extractContent(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, boolean hovered, float partialTicks) {
		this.render(guiGraphics, this.index, this.getY(), this.getX(), this.getY() + this.getHeight(), this.getX() + this.getWidth(), mouseX, mouseY, hovered, partialTicks);
	}

	/** Legacy 1.20-style entry render hook, bridged from extractContent. */
	public void render(GuiGraphicsExtractor guiGraphics, int index, int top, int left, int bottom, int right, int mouseX, int mouseY, boolean p_render_8_, float partialTicks) {}
	...
	public boolean mouseClicked(net.minecraft.client.input.MouseButtonEvent lycEvent, boolean lycDoubleClick) {
		return this.mouseClicked(lycEvent.x(), lycEvent.y(), lycEvent.button());
	}
	...
	public boolean mouseReleased(net.minecraft.client.input.MouseButtonEvent lycEvent) {
		return this.mouseReleased(lycEvent.x(), lycEvent.y(), lycEvent.button());
	}
