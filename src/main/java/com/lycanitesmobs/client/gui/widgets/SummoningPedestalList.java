...
public class SummoningPedestalList extends BaseList<SummoningPedestalScreen> {
	...
	protected boolean isSelectedItem(int index) {
		if(!(this.children().get(index) instanceof SummoningPedestalEntry))
			return false;
		return this.screen.getSelectedMinionName() != null && this.screen.getSelectedMinionName().equals(((SummoningPedestalEntry)this.children().get(index)).minionName);
	}
	...
	public class SummoningPedestalEntry extends BaseListEntry {
		...
		public void render(GuiGraphicsExtractor matrixStack, int index, int top, int left, int bottom, int right, int mouseX, int mouseY, boolean focus, float partialTicks) {
			...
			this.parentGUI.screen.drawHelper.draw(matrixStack, creatureInfo.getTitle().getString(), left + 20 , top + 4, 0xFFFFFF);
