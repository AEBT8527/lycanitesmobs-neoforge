...
public class CreatureRecolorScreen extends BaseScreen {
    ...
    protected void renderForeground(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTicks) {
        ...
        guiGraphics.textRenderer().accept(
                leftGroup.x,
                leftGroup.y - 18,
                net.minecraft.network.chat.Component.literal("Editing: " + this.creature.getName().getString()).withColor(0xFFFFFF)
        );
        int cardTop = rightGroup.y + 12;
        int cardBottom = rightGroup.y + rightGroup.height - 28;
        int cardLeft = rightGroup.x;
    ...
    @Override
    public boolean mouseClicked(net.minecraft.client.input.MouseButtonEvent lycEvent, boolean lycDoubleClick) {
        double mouseX = lycEvent.x();
        double mouseY = lycEvent.y();
        int button = lycEvent.button();
        if (button == 0 && !lycEvent.hasShiftDown() && insidePreview(mouseX, mouseY)) {
        ...
        if (button == 0 && lycEvent.hasShiftDown()) {
        ...
        return super.mouseClicked(lycEvent, lycDoubleClick);
    }

    @Override
    public boolean mouseDragged(net.minecraft.client.input.MouseButtonEvent lycEvent, double dx, double dy) {
        double mouseX = lycEvent.x();
        double mouseY = lycEvent.y();
        int button = lycEvent.button();
        ...
        return super.mouseDragged(lycEvent, dx, dy);
    }


    @Override
    public boolean mouseReleased(net.minecraft.client.input.MouseButtonEvent lycEvent) {
        double mouseX = lycEvent.x();
        double mouseY = lycEvent.y();
        int button = lycEvent.button();
        if (button == 0) {
        ...
        return super.mouseReleased(lycEvent);
    }
    ...
    private void renderResizeHandles(GuiGraphicsExtractor guiGraphics) {
        int size = 10;
        int padding = 4;
