...
public abstract class BeastiaryScreen extends BaseScreen {
    ...
    @Override
    public void renderBackground(GuiGraphicsExtractor matrixStack, int mouseX, int mouseY, float partialTicks) {
    ...
    @Override
    public void renderForeground(GuiGraphicsExtractor matrixStack, int mouseX, int mouseY, float partialTicks) {
     ...
     */
    @Override
    public boolean keyReleased(net.minecraft.client.input.KeyEvent lycEvent) {
        int keyCode = lycEvent.key();
        if (keyCode == 1 || keyCode == this.mc.options.keyInventory.getKey().getValue()) {
            this.mc.player.closeContainer();
        }
        return super.keyReleased(lycEvent);
    }
     ...
     */
    public void drawLevel(GuiGraphicsExtractor matrixStack, CreatureInfo creatureInfo, Identifier texture, int x, int y) {
    ...
    public void renderCreature(GuiGraphicsExtractor matrixStack, CreatureInfo creatureInfo, int x, int y, int mouseX, int mouseY, float partialTicks) {
        ...
        try {
            ...
            if (this.creaturePreviewEntity == null || this.creaturePreviewEntity.getClass() != creatureInfo.getEntityClass() || !subspeciesMatch) {
                this.creaturePreviewEntity = creatureInfo.createEntity(this.player.level());
    ...
    public void playCreatureSelectSound(CreatureInfo creatureInfo) {
        ...
        this.player.level().playSound(this.player, this.player.position().x(), this.player.position().y(), this.player.position().z(), soundEvent, SoundSource.NEUTRAL, 1, 1);
    }
