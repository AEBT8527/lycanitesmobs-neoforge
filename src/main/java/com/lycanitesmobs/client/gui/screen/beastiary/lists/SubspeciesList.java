...
public class SubspeciesList extends BaseList<BeastiaryScreen> {
	...
	protected boolean isSelectedItem(int index) {
		if(!(this.children().get(index) instanceof Entry))
			return false;
		if(!this.summoning) {
			return this.screen.playerExt.isSelectedSubspeciesVariant(((Entry)this.children().get(index)).subspeciesIndex, ((Entry)this.children().get(index)).variantIndex);
		}
		else {
			return this.screen.playerExt.getSelectedSummonSet().getSubspecies() == ((Entry)this.children().get(index)).subspeciesIndex &&
					this.screen.playerExt.getSelectedSummonSet().getVariant() == ((Entry)this.children().get(index)).variantIndex;
		}
	}
	...
	public static class Entry extends BaseListEntry {
		...
		@Override
		public void render(GuiGraphicsExtractor guiGraphics, int index, int top, int left, int bottom, int right, int mouseX, int mouseY, boolean focus, float partialTicks) {
