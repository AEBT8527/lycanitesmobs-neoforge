...
public class SummoningBeastiaryScreen extends BeastiaryScreen {
    ...
    @Override
    protected void renderWidgets(GuiGraphicsExtractor matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        this.petList.extractWidgetRenderState(matrixStack, mouseX, mouseY, partialTicks);
        this.subspeciesList.extractWidgetRenderState(matrixStack, mouseX, mouseY, partialTicks);
    ...
    public void playCreatureSelectSound(CreatureInfo creatureInfo) {
        this.player.level().playSound(this.player, this.player.position().x(), this.player.position().y(), this.player.position().z(), ObjectManager.getSound(creatureInfo.getName() + "_tame"), SoundSource.NEUTRAL, 1, 1);
    }
}
