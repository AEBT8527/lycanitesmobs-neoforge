...
public class PetTypeList extends CreatureFilterList {
	...
	public void setSelected(@Nullable BaseListEntry entry) {
		...
		this.screen.playerExt.selectPetType(this.getSelectedIndex());
	...
	protected boolean isSelectedItem(int index) {
		return this.screen.playerExt.getSelectedPetType() == index;
	}
	...
	@Override
	public void renderSlot(GuiGraphicsExtractor matrixStack, int index, int top, int left, int bottom, int right) {
	}
	...
	public static class Entry extends CreatureFilterList.Entry {
		...
		public void render(GuiGraphicsExtractor guiGraphics, int index, int top, int left, int bottom, int right, int mouseX, int mouseY, boolean focus, float partialTicks) {
			this.parentList.screen.drawHelper.drawString(guiGraphics, Component.translatable(this.petTypeKey).getString(), left + 2 , top + 4, 0xFFFFFF);
		}
