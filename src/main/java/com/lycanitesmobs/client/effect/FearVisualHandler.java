...
public class FearVisualHandler {
    ...
    public static void tick() {
        ...
        if (player != null) {
            ...
            if (fear != null && player.hasEffect(ObjectManager.holder(fear))) {
                MobEffectInstance instance = player.getEffect(ObjectManager.holder(fear));
    ...
    public static void saveCleanPixels(NativeImage pixels, DynamicTexture texture) {
        for (int sky = 0; sky < 16; sky++) {
            for (int block = 0; block < 16; block++) {
                cleanPixels[sky * 16 + block] = pixels.getPixel(block, sky);
            }
        }
    ...
    public static void saveDimmedPixels(NativeImage pixels) {
        for (int sky = 0; sky < 16; sky++) {
            for (int block = 0; block < 16; block++) {
                dimmedPixels[sky * 16 + block] = pixels.getPixel(block, sky);
            }
        }
        hasSavedPixels = true;
    }
    ...
    private static void uploadPixels(int[] source) {
        ...
        for (int sky = 0; sky < 16; sky++) {
            for (int block = 0; block < 16; block++) {
                pixels.setPixel(block, sky, source[sky * 16 + block]);
            }
        }
