...
public class ClientManager {
    ...
    public void registerEvents() {
        ...
        NeoForge.EVENT_BUS.addListener(clientEventListener::onPlayerTick);
        NeoForge.EVENT_BUS.addListener(clientEventListener::onKeyboardEvent);
        NeoForge.EVENT_BUS.addListener(clientEventListener::onMouseEvent);
        NeoForge.EVENT_BUS.addListener(clientEventListener::onItemTooltip);
        NeoForge.EVENT_BUS.addListener(clientEventListener::onFogDensity);
        NeoForge.EVENT_BUS.addListener(clientEventListener::onBlockOverlay);
        NeoForge.EVENT_BUS.addListener(ClientEventListener::onAfterSky);
        NeoForge.EVENT_BUS.addListener(ClientEventListener::onAfterLevel);
    ...
    public Font getFontRenderer() {
		...
		/*if(this.fontRenderer == null) {
			Identifier fontResource = Identifier.fromNamespaceAndPath(LycanitesMobs.MODID, "fonts/diavlo_light.otf");
    ...
    public void initBlockRenderTypes() {
        // 26.x: block render layers come from the model/blockstate definitions; nothing to register here.
    }
}